...
export default function AdminCatalogsClient() {
  ...
  return (
    <div className="space-y-5">
      ...
      {success && <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm nf-success-fg">{success}</div>}
      {error && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm nf-danger-fg">{error}</div>}
      <div className="grid gap-5 lg:grid-cols-[240px_1fr]">
        ...
        <Card>
          ...
          {visible.length === 0 ? <div className="rounded-xl border border-dashed border-gray-300 px-6 py-12 text-center"><p className="text-sm font-medium nf-text-secondary-fg">No hay valores configurados</p><p className="mt-1 text-xs nf-text-subtle-fg">Crea el primero para usarlo en los módulos de NormaFlow.</p></div> : <div className="divide-y divide-gray-100 rounded-xl border nf-border-line">{visible.map((row) => <div key={row.id} className="flex items-center justify-between gap-3 px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium nf-text-primary-fg">{row.name}</p>{row.description && <p className="truncate text-xs nf-text-subtle-fg">{row.description}</p>}</div><div className="flex items-center gap-2"><Badge status={row.active ? "ACTIVE" : "OBSOLETE"} label={row.active ? "Activo" : "Inactivo"} />{canEdit && <><button type="button" className="rounded-md p-2 nf-text-subtle-fg hover:nf-surface-sunken-bg" title="Editar" onClick={() => { setEditing(row); setError(""); }}><Pencil size={15} /></button><button type="button" className="rounded-md p-2 nf-text-subtle-fg hover:nf-surface-sunken-bg" title={row.active ? "Desactivar" : "Activar"} onClick={() => startTransition(async () => { try { await admin.updateAdminCatalogItem?.({ id: row.id, active: !row.active }); setSuccess(row.active ? "Elemento desactivado." : "Elemento activado."); } catch (err: unknown) { setError(err instanceof Error ? err.message : "No se pudo actualizar el elemento."); } })}><Power size={15} /></button></>}</div></div>)}</div>}
        </Card>
      </div>
