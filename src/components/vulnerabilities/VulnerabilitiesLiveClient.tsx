  ...
  return <div>
    ...
    <Card>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
        <div style={{ position: "relative", flex: 1, minWidth: 200 }}><Search size={15} style={{ position: "absolute", left: 10, top: 11, color: "var(--nf-ink-3)" }} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar código, CVE o descripción…" className="nf-app-input" style={{ paddingLeft: 32 }} /></div>
  ...
  return <div className="nf-modal-backdrop" role="dialog" aria-modal="true"><div className="nf-modal" style={{ maxWidth: 760, width: "calc(100% - 32px)", maxHeight: "92vh", overflow: "auto" }}>
    ...
    <div style={{ display: "grid", gap: 16, padding: 20 }}>
      ...
      <Section title="Activos expuestos">
        ...
        {initial.canUpdate && <div style={{ display: "flex", gap: 8, marginTop: 6 }}><select className="nf-app-input" value={assetId} onChange={(e) => setAssetId(e.target.value)}><option value="">Activo…</option>{initial.assetOptions.map((o) => <option key={o.id} value={o.id}>{o.code} · {o.name}</option>)}</select><button type="button" className="nf-app-btn-ghost" disabled={pending || !assetId} onClick={() => onRun(() => linkVulnerabilityAsset({ vulnerabilityId: vuln.id, assetId }), { successMessage: "Activo vinculado." })}>Añadir</button></div>}
      </Section>
