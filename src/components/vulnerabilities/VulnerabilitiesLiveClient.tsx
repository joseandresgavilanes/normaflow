...
export default function VulnerabilitiesLiveClient({ initial }: { initial: VulnerabilitiesPayload }) {
  ...
  return <div>
    ...
    <div className="nf-metric-strip">
      ...
      <Metric label="Abiertas" value={initial.summary.open} icon={<Bug size={19} />} color="var(--nf-warning-text)" />
      <Metric label="Críticas abiertas" value={initial.summary.critical} icon={<Bug size={19} />} color="var(--nf-danger-text)" />
      <Metric label="Vencidas" value={initial.summary.overdue} icon={<Bug size={19} />} color="var(--nf-danger-text)" />
    </div>
    <Card>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
        ...
        {initial.canExport && <><Picker aria-label="Tipo de informe" className="nf-app-input" value={reportType} onChange={(e) => setReportType(e.target.value)} style={{ maxWidth: 160 }}><option value="open-vulnerabilities">Abiertas</option><option value="remediation-plan">Plan remediación</option></Picker><button type="button" className="nf-app-btn-ghost" disabled={exporting} onClick={() => void exportReport("EXCEL")}><Download size={14} />Excel</button><button type="button" className="nf-app-btn-ghost" disabled={exporting} onClick={() => void exportReport("PDF")}><Download size={14} />PDF</button></>}
      </div>
...
function VulnDetail({ vuln, initial, pending, onClose, onRun }: { vuln: Vuln; initial: VulnerabilitiesPayload; pending: boolean; onClose: () => void; onRun: ReturnType<typeof useServerAction>["run"] }) {
  ...
  return <div className="nf-modal-backdrop" role="dialog" aria-modal="true"><div className="nf-modal" style={{ maxWidth: 760, width: "calc(100% - 32px)", maxHeight: "92vh", overflow: "auto" }}>
    ...
    <div style={{ display: "grid", gap: 16, padding: 20 }}>
      ...
      {initial.canUpdate && <div style={{ display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" }}><label style={{ fontSize: 11 }}>Estado<Picker aria-label="Estado" className="nf-app-input" value={status} onChange={(e) => setStatus(e.target.value as typeof status)}>{Object.entries(STATUS_LABEL).map(([v, l]) => <option key={v} value={v}>{l}</option>)}</Picker></label><button type="button" className="nf-app-btn-ghost" disabled={pending} onClick={() => onRun(() => updateVulnerability({ id: vuln.id, code: vuln.code, source: vuln.source, cve: vuln.cve || null, severity: vuln.severity as never, exposure: vuln.exposure || undefined, description: vuln.description || undefined, responsibleId: vuln.responsible?.id ?? null, targetDate: vuln.targetDate || null, status: status as never }), { successMessage: "Estado actualizado." })}>Guardar estado</button></div>}
      ...
      <Section title="Activos expuestos">
        ...
        {initial.canUpdate && <div style={{ display: "flex", gap: 8, marginTop: 6 }}><Picker aria-label="Activo" className="nf-app-input" value={assetId} onChange={(e) => setAssetId(e.target.value)}><option value="">Activo…</option>{initial.assetOptions.map((o) => <option key={o.id} value={o.id}>{o.code} · {o.name}</option>)}</Picker><button type="button" className="nf-app-btn-ghost" disabled={pending || !assetId} onClick={() => onRun(() => linkVulnerabilityAsset({ vulnerabilityId: vuln.id, assetId }), { successMessage: "Activo vinculado." })}>Añadir</button></div>}
      </Section>
