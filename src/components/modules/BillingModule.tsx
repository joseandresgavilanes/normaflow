...
export default function BillingModule({ liveData }: { liveData?: BillingPayload }) {
  ...
  return <div>
    ...
    <div className="nf-kpi-summary" style={{ marginBottom: 18 }}><div className="nf-kpi-summary-cell"><CreditCard size={22} color="var(--nf-primary-active)" /><div><strong style={{ fontSize: 20, color: "var(--nf-ink, #0f1b2d)" }}>{currentPlan.name}</strong><div className="nf-app-help">{money(currentPlan.price, currentPlan.currency)}{currentPlan.price == null ? "" : " / mes"}</div></div></div><div className="nf-kpi-summary-cell"><CalendarDays size={22} color="var(--nf-primary-active)" /><div><strong style={{ color: "var(--nf-ink, #0f1b2d)" }}>{liveData?.currentPeriodEnd ? new Date(liveData.currentPeriodEnd).toLocaleDateString("es-ES") : trialActive ? "Trial" : "Sin fecha"}</strong><div className="nf-app-help">{trialActive ? "Fin del trial" : "Fin del periodo"}</div></div></div><div className="nf-kpi-summary-cell"><HardDrive size={22} color="var(--nf-success-text)" /><div><strong style={{ color: "var(--nf-ink, #0f1b2d)" }}>{usedGb.toFixed(2)} GB</strong><div className="nf-app-help">de {usage.storageLimitGb == null ? "ilimitado" : `${usage.storageLimitGb} GB`}</div></div></div><div className="nf-kpi-summary-cell"><Users size={22} color="var(--nf-success-text)" /><div><strong style={{ color: "var(--nf-ink, #0f1b2d)" }}>{usage.users}{usage.userLimit != null ? ` / ${usage.userLimit}` : ""}</strong><div className="nf-app-help">Usuarios activos</div></div></div></div>
