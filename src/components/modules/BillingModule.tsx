...
export default function BillingModule({ liveData }: { liveData?: BillingPayload }) {
  ...
  return <div>
    ...
    {live && trialActive && <Card style={{ padding: 17, marginBottom: 18, borderColor: "var(--nf-primary)", background: "#f3f5ff" }}><strong style={{ color: "#3344bd" }}>Trial activo · {liveData.trialDaysRemaining ?? 0} días restantes</strong><span style={{ display: "block", marginTop: 5, color: "#5e6b7a", fontSize: 13 }}>Prueba NormaFlow sin tarjeta. Elige un plan cuando quieras conservar el acceso después del trial.</span></Card>}
    {live && !liveData.stripeConfigured && <Card style={{ padding: 16, marginBottom: 18, borderColor: "var(--nf-warning)" }}><strong style={{ color: "#9a6510" }}>Stripe no está configurado.</strong><span style={{ marginLeft: 8, color: "var(--nf-ink-3)", fontSize: 13 }}>Checkout, portal y sincronización de webhooks permanecerán deshabilitados hasta configurar las variables Stripe.</span></Card>}
