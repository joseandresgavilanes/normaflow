...
function metric(icon: ReactNode, label: string, value: string) {
  return <div style={{ background: "#f8fafc", border: "1px solid var(--nf-line, #b8c8d9)", borderRadius: 12, padding: 14, display: "flex", gap: 12, alignItems: "center" }}>
    <span style={{ width: 38, height: 38, borderRadius: 10, background: "rgba(82, 102, 246,.08)", display: "grid", placeItems: "center", color: "#5266F6" }}>{icon}</span>
    <span><small style={{ display: "block", color: "var(--nf-ink-3, #314456)", fontWeight: 700, textTransform: "none" }}>{label}</small><strong style={{ color: "var(--nf-ink, #0f1b2d)" }}>{value}</strong></span>
  </div>;
}
...
export default function BillingModule({ liveData }: { liveData?: BillingPayload }) {
  ...
  return <div>
    ...
    {live && !liveData.stripeConfigured && <Card style={{ padding: 16, marginBottom: 18, borderColor: "#D97706" }}>
    ...
    <div className="nf-kpi-summary" style={{ marginBottom: 18 }}>
      <div className="nf-kpi-summary-cell"><CreditCard size={22} color="#5266F6" /><div><strong style={{ fontSize: 20, color: "var(--nf-ink, #0f1b2d)" }}>{currentPlan.name}</strong><div className="nf-app-help">{currentPlan.price}{currentPlan.period}</div></div></div>
      <div className="nf-kpi-summary-cell"><CalendarDays size={22} color="#5266F6" /><div><strong style={{ color: "var(--nf-ink, #0f1b2d)" }}>{liveData?.currentPeriodEnd ? new Date(liveData.currentPeriodEnd).toLocaleDateString("es-ES") : "Sin fecha"}</strong><div className="nf-app-help">Fin del periodo</div></div></div>
      <div className="nf-kpi-summary-cell"><HardDrive size={22} color="#16A34A" /><div><strong style={{ color: "var(--nf-ink, #0f1b2d)" }}>{storagePct.toFixed(1)}%</strong><div className="nf-app-help">Almacenamiento usado</div></div></div>
      <div className="nf-kpi-summary-cell"><Users size={22} color="#16A34A" /><div><strong style={{ color: "var(--nf-ink, #0f1b2d)" }}>{usage.users}</strong><div className="nf-app-help">Miembros</div></div></div>
    </div>
    ...
    <Card style={{ marginBottom: 22, padding: 20 }}>
      ...
      <ProgressBar value={storagePct} color="#16A34A" height={7} railColor="#eef2f9" />
    ...
    <h3 style={{ fontSize: 16, marginBottom: 14, fontWeight: 600, color: "var(--nf-ink, #0f1b2d)", letterSpacing: "-0.02em" }}>Planes</h3>
    <div className="nf-grid-stats nf-billing-plans" style={{ gap: 16, marginBottom: 24 }}>
      {PLANS.map(plan => {
        ...
        return <Card key={plan.key} style={{ padding: 18, border: current ? "2px solid #5266F6" : "1px solid var(--nf-line, #b8c8d9)" }}>
          <strong style={{ fontSize: 18, color: "var(--nf-ink, #0f1b2d)" }}>{plan.name}</strong>
          <div style={{ fontSize: 27, fontWeight: 900, color: "#5266F6", margin: "8px 0" }}>{plan.price}<small style={{ fontSize: 12 }}>{plan.period}</small></div>
          <ul style={{ paddingLeft: 0, listStyle: "none" }}>{plan.features.map(item => <li key={item} style={{ display: "flex", gap: 7, margin: "8px 0", fontSize: 13 }}><Check size={15} color="#16A34A" />{item}</li>)}</ul>
    ...
    <h3 style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 16, fontWeight: 600, color: "var(--nf-ink, #0f1b2d)", letterSpacing: "-0.02em" }}><Receipt size={18} />Facturas</h3>
