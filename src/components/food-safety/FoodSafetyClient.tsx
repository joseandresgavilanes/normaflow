...
function FoodSafetyRecordEditor({ kind, value, products, materials, flows, steps, hazards, assessments, ccps, oprps, plans, members, onCancel, onSave }: {
  ...
  const ref = (key: string, options: { id: string; label: string }[], placeholder: string) => <Picker aria-label={placeholder} style={input} value={form[key] ?? ""} onChange={(e) => set(key, e.target.value)}><option value="">{placeholder}</option>{options.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</Picker>;
  const select = (key: string, options: string[]) => <Picker aria-label={labelForKeyOrRaw(key)} style={input} value={form[key] ?? ""} onChange={(e) => set(key, e.target.value)}>{options.map((option) => <option key={option} value={option}>{option}</option>)}</Picker>;
...
function NewIntendedUseForm({ products, pending, run, onDone }: { products: FoodSafetyPayload["products"]; pending: boolean; run: Runner; onDone: () => void }) {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <FoodModalField label="Producto"><Picker aria-label="Seleccionar producto" style={input} value={f.productId} onChange={(e) => set("productId", e.target.value)}><option value="">Seleccionar producto…</option>{products.map((p) => <option key={p.id} value={p.id}>{p.code} — {p.name}</option>)}</Picker></FoodModalField>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr", gap: 8 }}>
        <Picker aria-label="Producto" style={input} value={f.productId} onChange={(e) => set("productId", e.target.value)}><option value="">Producto…</option>{products.map((p) => <option key={p.id} value={p.id}>{p.code} — {p.name}</option>)}</Picker>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 2fr 1fr", gap: 8 }}>
        <Picker aria-label="Flujo" style={input} value={f.flowId} onChange={(e) => set("flowId", e.target.value)}><option value="">Flujo…</option>{flows.map((fl) => <option key={fl.id} value={fl.id}>{fl.code} v{fl.version}</option>)}</Picker>
        ...
        <Picker aria-label="Tipo de paso" style={input} value={f.stepType} onChange={(e) => set("stepType", e.target.value)}>{["RECEIPT", "STORAGE", "PREP", "PROCESS", "COOKING", "COOLING", "PACKAGING", "DISTRIBUTION", "OTHER"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Tipo de peligro" style={input} value={f.hazardType} onChange={(e) => set("hazardType", e.target.value)}>{["BIOLOGICAL", "CHEMICAL", "PHYSICAL", "ALLERGEN"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
      </div>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        <Picker aria-label="Peligro" style={input} value={f.hazardId} onChange={(e) => set("hazardId", e.target.value)}><option value="">Peligro…</option>{hazards.map((h) => <option key={h.id} value={h.id}>{h.code} — {h.name}</option>)}</Picker>
        <Picker aria-label="Etapa" style={input} value={f.stepId} onChange={(e) => set("stepId", e.target.value)}><option value="">Etapa…</option>{steps.map((st) => <option key={st.id} value={st.id}>{st.code} — {st.name}</option>)}</Picker>
        <Picker aria-label="Producto" style={input} value={f.productId} onChange={(e) => set("productId", e.target.value)}><option value="">Producto…</option>{products.map((p) => <option key={p.id} value={p.id}>{p.code}</option>)}</Picker>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <Picker aria-label="Severidad" style={input} value={f.severity} onChange={(e) => set("severity", Number(e.target.value))}>{[1, 2, 3, 4, 5].map((v) => <option key={v} value={v}>Severidad {v}</option>)}</Picker>
        <Picker aria-label="Probabilidad" style={input} value={f.likelihood} onChange={(e) => set("likelihood", Number(e.target.value))}>{[1, 2, 3, 4, 5].map((v) => <option key={v} value={v}>Probabilidad {v}</option>)}</Picker>
      </div>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Categoría" style={input} value={f.category} onChange={(e) => set("category", e.target.value)}>{["HYGIENE", "PEST_CONTROL", "WATER", "CLEANING", "MAINTENANCE", "PERSONNEL", "SUPPLIER", "WASTE", "ALLERGEN_CONTROL", "OTHER"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
...
function NewOprpForm({ assessments, steps, members, pending, run, onDone }: {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        <Picker aria-label="Evaluación de peligro" style={input} value={f.hazardAssessmentId} onChange={(e) => set("hazardAssessmentId", e.target.value)}><option value="">Evaluación de peligro…</option>{assessments.map((a) => <option key={a.id} value={a.id}>{a.code}</option>)}</Picker>
        <Picker aria-label="Etapa" style={input} value={f.stepId} onChange={(e) => set("stepId", e.target.value)}><option value="">Etapa…</option>{steps.map((st) => <option key={st.id} value={st.id}>{st.code}</option>)}</Picker>
        <PersonPicker people={members} value={f.responsibleId} onValueChange={(personId) => set("responsibleId", personId)} placeholder="Responsable…" ariaLabel="Responsable" style={input} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Etapa" style={input} value={f.stepId} onChange={(e) => set("stepId", e.target.value)}><option value="">Etapa…</option>{steps.map((st) => <option key={st.id} value={st.id}>{st.code} — {st.name}</option>)}</Picker>
        <Picker aria-label="Evaluación" style={input} value={f.hazardAssessmentId} onChange={(e) => set("hazardAssessmentId", e.target.value)}><option value="">Evaluación…</option>{assessments.map((a) => <option key={a.id} value={a.id}>{a.code}</option>)}</Picker>
      </div>
...
function NewLimitForm({ ccps, pending, run, onDone }: { ccps: FoodSafetyPayload["ccps"]; pending: boolean; run: Runner; onDone: () => void }) {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        <Picker aria-label="PCC" style={input} value={f.ccpId} onChange={(e) => set("ccpId", e.target.value)}><option value="">PCC…</option>{ccps.map((c) => <option key={c.id} value={c.id}>{c.code}</option>)}</Picker>
        <input aria-label="Parámetro" style={input} placeholder="Parámetro" value={f.parameter} onChange={(e) => set("parameter", e.target.value)} />
        <Picker aria-label="Operador" style={input} value={f.operator} onChange={(e) => set("operator", e.target.value)}>{["LT", "LTE", "GT", "GTE", "EQ", "BETWEEN"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
      </div>
...
function NewMonitoringPlanForm({ ccps, oprps, members, pending, run, onDone }: {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <Picker aria-label="PCC" style={input} value={f.ccpId} onChange={(e) => set("ccpId", e.target.value)}><option value="">PCC…</option>{ccps.map((c) => <option key={c.id} value={c.id}>{c.code}</option>)}</Picker>
        <Picker aria-label="OPRP" style={input} value={f.oprpId} onChange={(e) => set("oprpId", e.target.value)}><option value="">OPRP…</option>{oprps.map((o) => <option key={o.id} value={o.id}>{o.code}</option>)}</Picker>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
...
function NewMonitoringRecordForm({ plans, pending, run, onDone }: { plans: FoodSafetyPayload["plans"]; pending: boolean; run: Runner; onDone: () => void }) {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        <Picker aria-label="Plan de monitoreo" style={input} value={f.planId} onChange={(e) => set("planId", e.target.value)}><option value="">Plan de monitoreo…</option>{plans.map((p) => <option key={p.id} value={p.id}>{p.code} — {p.title}</option>)}</Picker>
...
function NewDeviationForm({ ccps, pending, run, onDone }: { ccps: FoodSafetyPayload["ccps"]; pending: boolean; run: Runner; onDone: () => void }) {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        <Picker aria-label="PCC" style={input} value={f.ccpId} onChange={(e) => set("ccpId", e.target.value)}><option value="">PCC…</option>{ccps.map((c) => <option key={c.id} value={c.id}>{c.code}</option>)}</Picker>
        <Picker aria-label="Severidad" style={input} value={f.severity} onChange={(e) => set("severity", e.target.value)}>{["MINOR", "MODERATE", "MAJOR", "CRITICAL"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
...
function NewCorrectionForm({ deviations, pending, run, onDone }: { deviations: FoodSafetyPayload["deviations"]; pending: boolean; run: Runner; onDone: () => void }) {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <Picker aria-label="Desviación" style={input} value={f.deviationId} onChange={(e) => set("deviationId", e.target.value)}><option value="">Desviación…</option>{deviations.map((d) => <option key={d.id} value={d.id}>{d.code} — {d.title}</option>)}</Picker>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Tipo de objetivo" style={input} value={f.targetType} onChange={(e) => set("targetType", e.target.value)}>{["CCP", "OPRP", "PRP", "PROCESS", "OTHER"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 8 }}>
        <Picker aria-label="Resultado" style={input} value={f.result} onChange={(e) => set("result", e.target.value)}>{["PENDING", "VALID", "INVALID", "CONDITIONAL"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Tipo de actividad" style={input} value={f.activityType} onChange={(e) => set("activityType", e.target.value)}>{["INTERNAL_AUDIT", "RECORD_REVIEW", "CALIBRATION_CHECK", "SAMPLING", "SUPPLIER_AUDIT", "OTHER"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
        <DateField aria-label="Fecha prevista" style={input} value={f.scheduledFor} onChange={(e) => set("scheduledFor", e.target.value)} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 2fr", gap: 8 }}>
        <Picker aria-label="Resultado" style={input} value={f.result} onChange={(e) => set("result", e.target.value)}>{["PENDING", "CONFORMING", "NONCONFORMING", "PARTIAL"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
        <PersonPicker people={members} value={f.responsibleId} onValueChange={(personId) => set("responsibleId", personId)} placeholder="Responsable…" ariaLabel="Responsable" style={input} />
        <input aria-label="Hallazgos" style={input} placeholder="Hallazgos" value={f.findings} onChange={(e) => set("findings", e.target.value)} />
      </div>
...
function NewLotForm({ products, materials, lots, pending, run, onDone }: {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        <Picker aria-label="Tipo de lote" style={input} value={f.lotType} onChange={(e) => set("lotType", e.target.value)}>{["RAW_MATERIAL", "INTERMEDIATE", "FINISHED", "DISTRIBUTED"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
        <Picker aria-label="Producto" style={input} value={f.productId} onChange={(e) => set("productId", e.target.value)}><option value="">Producto…</option>{products.map((p) => <option key={p.id} value={p.id}>{p.code}</option>)}</Picker>
        <Picker aria-label="Materia prima" style={input} value={f.rawMaterialId} onChange={(e) => set("rawMaterialId", e.target.value)}><option value="">Materia prima…</option>{materials.map((m) => <option key={m.id} value={m.id}>{m.code}</option>)}</Picker>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 8 }}>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Tipo de retirada" style={input} value={f.recallType} onChange={(e) => set("recallType", e.target.value)}>{["WITHDRAWAL", "RECALL", "STOCK_RECOVERY"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
      </div>
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Tipo de emergencia" style={input} value={f.emergencyType} onChange={(e) => set("emergencyType", e.target.value)}>{["CONTAMINATION", "ALLERGEN_INCIDENT", "RECALL_EVENT", "SUPPLY_DISRUPTION", "FACILITY", "OTHER"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
      </div>
      <input aria-label="Descripción" style={input} placeholder="Descripción" value={f.description} onChange={(e) => set("description", e.target.value)} />
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 8 }}>
        ...
        <Picker aria-label="Parte" style={input} value={f.party} onChange={(e) => set("party", e.target.value)}>{["SUPPLIER", "CUSTOMER", "AUTHORITY", "OTHER"].map((v) => <option key={v} value={v}>{v}</option>)}</Picker>
