...
function IncidentForm({ initial, pending, onClose, onRun }: { initial: IncidentsPayload; pending: boolean; onClose: () => void; onRun: ReturnType<typeof useServerAction>["run"] }) {
  ...
  return <div className="nf-modal-backdrop" role="dialog" aria-modal="true"><div className="nf-modal" style={{ maxWidth: 640, width: "calc(100% - 32px)", maxHeight: "90vh", overflow: "auto" }}>
    ...
    <div style={{ display: "grid", gap: 12, padding: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><label>Código<input className="nf-app-input" value={f.code} onChange={(e) => set("code", e.target.value)} /></label><label>Detectado<input className="nf-app-input" type="date" value={f.detectedAt} onChange={(e) => set("detectedAt", e.target.value)} /></label></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        ...
        <label>Categoría<select className="nf-app-input" value={f.category} onChange={(e) => set("category", e.target.value)}>{Object.entries(CAT_LABEL).map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></label>
