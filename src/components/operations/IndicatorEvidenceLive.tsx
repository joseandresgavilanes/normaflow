...
export function IndicatorsLiveClient({ initial }: { initial: IndicatorsPayload }) {
  ...
  return <div><OperationalHeader title="Indicadores" subtitle={`${initial.indicators.length} indicadores persistidos`} canCreate={initial.access.canCreate} actionLabel="Nuevo indicador" onCreate={() => { setError(""); setCreating(true); }} /><OperationalMessages error={error} success={success} />
    {initial.indicators.length === 0 ? <EmptyOperational>No hay indicadores registrados.</EmptyOperational> : <OperationalGrid>{initial.indicators.map((indicator) => { const latest = indicator.values[0]; return <OperationalCard key={indicator.id} onClick={() => setDetail(indicator)}><div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><div style={{ fontSize: 11, fontWeight: 600, color: "var(--nf-ink-3)" }}>{indicator.frequency} · {indicator.processCode ?? "Sin proceso"}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{indicator.name}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{indicator.ownerName ?? "Sin responsable"}</div></div><Badge status={indicator.status} /></div><div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 14 }}><strong style={{ fontSize: 28, color: "#5266F6" }}>{latest?.value ?? "—"}</strong><span style={{ color: "var(--nf-ink-3)" }}>{indicator.unit} / meta {indicator.target}</span></div><CardActions canUpdate={initial.access.canUpdate} canDelete={initial.access.canDelete} pending={isPending} onEdit={() => { setError(""); setEditing(indicator); }} onDelete={() => remove(indicator)} />{initial.access.canUpdate && <button type="button" className="nf-app-btn-primary" style={{ marginTop: 9, width: "100%" }} onClick={(event) => { event.stopPropagation(); setValueIndicator(indicator); }}>Registrar medición</button>}</OperationalCard>; })}</OperationalGrid>}
    <FormModal open={creating || !!editing} title={editing ? "Editar indicador" : "Nuevo indicador"} pending={isPending} error={error} onClose={() => { setCreating(false); setEditing(null); setError(""); }} onSubmit={submit}><Field label="Nombre"><input name="name" required className="nf-app-input" style={inputStyle} defaultValue={row?.name ?? ""} /></Field><div className="nf-grid-2" style={{ gap: 12 }}><Field label="Unidad"><input name="unit" required className="nf-app-input" style={inputStyle} defaultValue={row?.unit ?? "%"} /></Field><Field label="Meta"><input name="target" type="number" step="any" required className="nf-app-input" style={inputStyle} defaultValue={row?.target ?? 100} /></Field></div><div className="nf-grid-2" style={{ gap: 12 }}><Field label="Frecuencia"><select name="frequency" className="nf-app-input" style={inputStyle} defaultValue={row?.frequency ?? "monthly"}><option value="daily">Diaria</option><option value="weekly">Semanal</option><option value="monthly">Mensual</option><option value="quarterly">Trimestral</option><option value="annual">Anual</option></select></Field><Field label="Estado"><select name="status" className="nf-app-input" style={inputStyle} defaultValue={row?.status ?? IndicatorStatus.ON_TRACK}>{Object.values(IndicatorStatus).map((value) => <option key={value}>{value}</option>)}</select></Field></div><div className="nf-grid-2" style={{ gap: 12 }}><Field label="Proceso"><select name="processId" className="nf-app-input" style={inputStyle} defaultValue={row?.processId ?? ""}><option value="">Sin proceso</option>{initial.processes.map((process) => <option key={process.id} value={process.id}>{process.code ?? "PROC"} · {process.name}</option>)}</select></Field><Field label="Responsable"><select name="ownerId" className="nf-app-input" style={inputStyle} defaultValue={row?.ownerId ?? ""}><option value="">Sin asignar</option>{initial.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field></div><Field label="Cláusula"><input name="clauseCode" className="nf-app-input" style={inputStyle} defaultValue={row?.clauseCode ?? ""} /></Field><Field label="Descripción"><textarea name="description" rows={3} className="nf-app-input" style={inputStyle} defaultValue={row?.description ?? ""} /></Field></FormModal>
    <FormModal open={!!valueIndicator} title={`Nueva medición · ${valueIndicator?.name ?? ""}`} pending={isPending} error={error} onClose={() => setValueIndicator(null)} onSubmit={submitValue}><div className="nf-grid-2" style={{ gap: 12 }}><Field label="Valor"><input name="value" type="number" step="any" required className="nf-app-input" style={inputStyle} /></Field><Field label="Periodo"><input name="period" placeholder="2026-06" required className="nf-app-input" style={inputStyle} /></Field></div><Field label="Nota"><textarea name="note" rows={3} className="nf-app-input" style={inputStyle} /></Field></FormModal>
...
export function EvidenceLiveClient({ initial }: { initial: EvidencePayload }) {
  ...
  return <div><OperationalHeader title="Evidencias" subtitle={`${initial.evidence.length} evidencias en base de datos y Storage`} canCreate={initial.access.canCreate} actionLabel="Subir evidencia" onCreate={() => { setError(""); setCreating(true); }} /><OperationalMessages error={error} success={success} />
    {initial.evidence.length === 0 ? <EmptyOperational>No hay evidencias cargadas.</EmptyOperational> : <OperationalGrid>{initial.evidence.map((item) => <OperationalCard key={item.id}><div style={{ fontSize: 11, fontWeight: 600, color: "var(--nf-ink-3)", textTransform: "none" }}>{item.module ?? "General"}{item.targetLabel ? ` · ${item.targetLabel}` : ""}</div><h3 style={{ margin: "7px 0", fontSize: 17 }}>{item.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{item.mimeType ?? "Archivo"} · {item.fileSize ? `${Math.ceil(item.fileSize / 1024)} KB` : "Tamaño desconocido"} · {new Date(item.createdAt).toLocaleDateString("es")}</div><div style={{ display: "flex", gap: 8, marginTop: 14 }}><button type="button" className="nf-app-btn-primary" disabled={opening === item.id} onClick={() => openEvidence(item)}>{opening === item.id ? "Abriendo…" : "Abrir"}</button>{initial.access.canDelete && <button type="button" className="nf-app-btn-outline" disabled={isPending} onClick={() => remove(item)} style={{ color: "#a62d29", borderColor: "#efc5c3" }}>Eliminar</button>}</div></OperationalCard>)}</OperationalGrid>}
    <FormModal open={creating} title="Subir evidencia" pending={isPending} error={error} onClose={() => { setCreating(false); setModule(""); setError(""); }} onSubmit={submit} submitLabel="Subir"><Field label="Título"><input name="title" required className="nf-app-input" style={inputStyle} /></Field><Field label="Módulo relacionado"><select name="module" className="nf-app-input" style={inputStyle} value={module} onChange={(event) => setModule(event.target.value as EvidenceModule | "")}><option value="">Evidencia general</option><option value="process">Proceso</option><option value="risk">Riesgo</option><option value="audit">Auditoría</option><option value="nc">No conformidad</option><option value="indicator">Indicador</option><option value="document">Documento</option><option value="change">Cambio</option><option value="supplier">Proveedor</option><option value="integration">Integración</option></select></Field>{module && <Field label="Registro relacionado"><select name="moduleId" required className="nf-app-input" style={inputStyle}><option value="">Seleccionar…</option>{targets.map((target) => <option key={target.id} value={target.id}>{target.label}</option>)}</select></Field>}<Field label="Archivo"><input name="file" type="file" required className="nf-app-input" style={inputStyle} accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.csv,.txt,.md,image/*" /></Field></FormModal>
    <ConfirmActionModal
      open={!!confirmDelete}
