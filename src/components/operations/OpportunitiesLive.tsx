...
export function OpportunitiesLive({ initial }: { initial: OpportunitiesPayload }) {
  ...
  return <div>
    ...
    <FormModal open={creating || !!editing} title={editing ? "Editar oportunidad" : "Nueva oportunidad"} pending={isPending} error={error} onClose={() => { setCreating(false); setEditing(null); setError(""); }} onSubmit={submit}>
      ...
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Norma / versión"><input name="standardCode" className="nf-app-input" style={inputStyle} placeholder="ISO_9001:2015" defaultValue={row?.standardCode ?? ""} /></Field><Field label="Categoría"><input name="category" required className="nf-app-input" style={inputStyle} placeholder="Cliente, proceso, tecnología…" defaultValue={row?.category ?? ""} /></Field></div>
      <Field label="Descripción"><textarea name="description" rows={3} className="nf-app-input" style={inputStyle} defaultValue={row?.description ?? ""} /></Field>
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Fuente"><input name="source" className="nf-app-input" style={inputStyle} placeholder="Auditoría, análisis de contexto…" defaultValue={row?.source ?? ""} /></Field><Field label="Fecha objetivo"><input name="dueDate" type="date" className="nf-app-input" style={inputStyle} defaultValue={row?.dueDate?.slice(0, 10) ?? ""} /></Field></div>
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Responsable"><select name="ownerId" className="nf-app-input" style={inputStyle} defaultValue={row?.ownerId ?? ""}><option value="">Sin asignar</option>{memberRows.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field><Field label="Revisor obligatorio"><select name="reviewerId" required className="nf-app-input" style={inputStyle} defaultValue={row?.reviewerId ?? ""}><option value="">Seleccionar revisor…</option>{memberRows.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field></div>
      <Field label="Análisis de materialización"><textarea name="materializationAnalysis" rows={3} className="nf-app-input" style={inputStyle} placeholder="¿Qué evidencia demostrará que la oportunidad se materializó?" defaultValue={row?.materializationAnalysis ?? ""} /></Field>
      <Field label="Plan de materialización"><textarea name="materializationPlan" rows={3} className="nf-app-input" style={inputStyle} placeholder="Acciones, responsables, recursos y criterios de éxito." defaultValue={row?.materializationPlan ?? ""} /></Field>
      <Field label="Evidencia / resultado"><textarea name="materializationEvidence" rows={2} className="nf-app-input" style={inputStyle} defaultValue={row?.materializationEvidence ?? ""} /></Field>
    </FormModal>
