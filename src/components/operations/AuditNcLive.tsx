...
export function NonconformitiesLiveClient({ initial }: { initial: NonconformitiesPayload }) {
  ...
  return <div><OperationalHeader title="No conformidades" subtitle={`${initial.nonconformities.length} NC persistidas`} canCreate={initial.access.canCreate} actionLabel="Nueva NC" onCreate={() => { setError(""); setCreating(true); }} /><OperationalMessages error={error} success={success} />
    {initial.nonconformities.length === 0 ? <EmptyOperational>No hay no conformidades registradas.</EmptyOperational> : <OperationalGrid>{initial.nonconformities.map((nc) => <OperationalCard key={nc.id} onClick={() => setDetail(nc)}><div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><div style={{ fontSize: 11, fontWeight: 600, color: "var(--nf-ink-3)" }}>{nc.source} · {nc.severity}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{nc.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{nc.auditTitle ?? "Sin auditoría"} · {nc.ownerName ?? "Sin responsable"}</div></div><Badge status={nc.status} /></div><div style={{ marginTop: 12, fontSize: 12, color: "var(--nf-ink-3)" }}>{nc.dueDate ? `Vence ${new Date(nc.dueDate).toLocaleDateString("es")}` : "Sin vencimiento"} · {nc.actionCount} acciones</div><CardActions canUpdate={initial.access.canUpdate} canDelete={initial.access.canDelete} pending={isPending} onEdit={() => { setError(""); setEditing(nc); }} onDelete={() => remove(nc)} /></OperationalCard>)}</OperationalGrid>}
