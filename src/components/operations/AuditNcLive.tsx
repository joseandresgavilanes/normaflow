...
export function AuditsLiveClient({ initial }: { initial: AuditsPayload }) {
  ...
  return <div>
    ...
    {initial.audits.length === 0 ? <EmptyOperational>No hay auditorías registradas.</EmptyOperational> : <OperationalGrid>{initial.audits.map((audit) => <OperationalCard key={audit.id} onClick={() => setDetail(audit)}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><div style={{ fontSize: 11, fontWeight: 800, color: "var(--nf-ink-3)" }}>{audit.type} · {audit.standardCode ?? "Sin norma"}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{audit.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{audit.auditorName ?? audit.auditorExternal ?? "Sin auditor"}</div></div><Badge status={audit.status} /></div>
      <div style={{ marginTop: 13, height: 7, borderRadius: 99, background: "#e7edf4", overflow: "hidden" }}><div style={{ width: `${audit.progress}%`, height: "100%", background: "#2E8B57" }} /></div>
...
export function NonconformitiesLiveClient({ initial }: { initial: NonconformitiesPayload }) {
  const { run, isPending, error, setError, success } = useServerAction();
  const [creating, setCreating] = useState(false); const [editing, setEditing] = useState<NcRow | null>(null); const [detail, setDetail] = useState<NcRow | null>(null);
  ...
  return <div><OperationalHeader title="No conformidades" subtitle={`${initial.nonconformities.length} NC persistidas`} canCreate={initial.access.canCreate} actionLabel="Nueva NC" onCreate={() => { setError(""); setCreating(true); }} /><OperationalMessages error={error} success={success} />
    {initial.nonconformities.length === 0 ? <EmptyOperational>No hay no conformidades registradas.</EmptyOperational> : <OperationalGrid>{initial.nonconformities.map((nc) => <OperationalCard key={nc.id} onClick={() => setDetail(nc)}><div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><div style={{ fontSize: 11, fontWeight: 800, color: "var(--nf-ink-3)" }}>{nc.source} · {nc.severity}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{nc.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{nc.auditTitle ?? "Sin auditoría"} · {nc.ownerName ?? "Sin responsable"}</div></div><Badge status={nc.status} /></div><div style={{ marginTop: 12, fontSize: 12, color: "var(--nf-ink-3)" }}>{nc.dueDate ? `Vence ${new Date(nc.dueDate).toLocaleDateString("es")}` : "Sin vencimiento"} · {nc.actionCount} acciones</div><CardActions canUpdate={initial.access.canUpdate} canDelete={initial.access.canDelete} pending={isPending} onEdit={() => { setError(""); setEditing(nc); }} onDelete={() => remove(nc)} /></OperationalCard>)}</OperationalGrid>}
    ...
    <Modal open={!!detail} onClose={() => setDetail(null)} title={detail?.title ?? "NC"} width={650}>{detail && <div style={{ display: "grid", gap: 18 }}><div className="nf-grid-2"><Meta label="Estado" value={detail.status} /><Meta label="Severidad" value={detail.severity} /><Meta label="Auditoría" value={detail.auditTitle} /><Meta label="Hallazgo" value={detail.findingTitle} /></div><Meta label="Descripción" value={detail.description} /><Meta label="Causa raíz" value={detail.rootCause} /><Meta label="Eficacia" value={detail.effectivenessValidated ? "Validada" : "Pendiente"} /></div>}</Modal>
  </div>;
}
