...
export function AuditsLiveClient({ initial }: { initial: AuditsPayload }) {
  ...
  return <div>
    ...
    <FormModal open={!!checklistItem} title="Responder checklist" pending={isPending} error={error} onClose={() => { setChecklistItem(null); setError(""); }} onSubmit={submitChecklistResponse}><Field label="Pregunta"><div style={{ fontSize: 13, fontWeight: 600 }}>{checklistItem?.question}</div></Field><Field label="Estado"><Picker aria-label="Estado" name="status" className="nf-app-input" style={inputStyle} defaultValue={checklistItem?.status ?? ChecklistItemStatus.PENDING}>{Object.values(ChecklistItemStatus).map((value) => <option key={value}>{value}</option>)}</Picker></Field><Field label="Respuesta"><textarea aria-label="Respuesta" name="response" rows={3} className="nf-app-input" style={inputStyle} defaultValue={checklistItem?.response ?? ""} /></Field><Field label="Notas"><textarea aria-label="Notas" name="notes" rows={2} className="nf-app-input" style={inputStyle} defaultValue={checklistItem?.notes ?? ""} /></Field><Field label="Evidencia del repositorio"><Picker aria-label="Evidencia del repositorio" name="evidenceId" className="nf-app-input" style={inputStyle} defaultValue={checklistItem?.evidenceId ?? ""}><option value="">Sin evidencia del repositorio</option>{initial.evidenceFiles.map((file) => <option key={file.id} value={file.id}>{file.title}</option>)}</Picker></Field><Field label="Enlace externo (opcional)"><input aria-label="Enlace externo a la evidencia" name="evidenceUrl" className="nf-app-input" style={inputStyle} defaultValue={checklistItem?.evidenceUrl ?? ""} /></Field></FormModal>
    <FormModal open={!!findingAudit} title="Registrar hallazgo" pending={isPending} error={error} onClose={() => setFindingAudit(null)} onSubmit={submitFinding}><Field label="Título"><input aria-label="Título" name="title" required className="nf-app-input" style={inputStyle} /></Field><div className="nf-grid-2" style={{ gap: 12 }}><Field label="Tipo"><Picker aria-label="Tipo" name="type" className="nf-app-input" style={inputStyle}>{Object.values(FindingType).map((value) => <option key={value}>{value}</option>)}</Picker></Field><Field label="Severidad"><Picker aria-label="Severidad" name="severity" className="nf-app-input" style={inputStyle}>{Object.values(FindingSeverity).map((value) => <option key={value}>{value}</option>)}</Picker></Field></div><Field label="Cláusula"><input aria-label="Código de cláusula" name="clauseCode" className="nf-app-input" style={inputStyle} /></Field><Field label="Descripción"><textarea aria-label="Descripción" name="description" rows={3} className="nf-app-input" style={inputStyle} /></Field><Field label="Evidencia del repositorio"><Picker aria-label="Evidencia del repositorio" name="evidenceId" className="nf-app-input" style={inputStyle} defaultValue=""><option value="">Sin evidencia del repositorio</option>{initial.evidenceFiles.map((file) => <option key={file.id} value={file.id}>{file.title}</option>)}</Picker></Field><Field label="Enlace externo (opcional)"><input aria-label="Enlace externo a la evidencia" name="evidenceUrl" className="nf-app-input" style={inputStyle} /></Field></FormModal>
