...
function emptyBulkAudit(id: number): BulkAuditDraft {
  return { id, title: "", type: AuditType.INTERNAL, standardCode: "ISO 9001", plannedDate: "", scheduledDate: "", auditorId: "", programId: "" };
}
...
export function AuditsLiveClient({ initial }: { initial: AuditsPayload }) {
  ...
  return <div>
    ...
    {showPlanning ? <section style={{ border: "1px solid var(--nf-line)", borderRadius: 14, padding: 16, background: "linear-gradient(135deg, #f7f9ff, #fff)" }}>
      ...
      {planningGroups.length ? <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(235px, 1fr))", gap: 12, marginTop: 16 }}>{planningGroups.map((group) => <div key={group.key} style={{ border: "1px solid var(--nf-line)", borderRadius: 11, background: "#fff", padding: 12 }}><div style={{ textTransform: "capitalize", fontSize: 12, fontWeight: 800, color: "#5266F6", marginBottom: 9 }}>{formatPlanningPeriod(group.date.toISOString())}</div><div style={{ display: "grid", gap: 8 }}>{group.audits.map((audit) => <button key={audit.id} type="button" onClick={() => setDetail(audit)} style={{ textAlign: "left", border: "1px solid #e8edf6", borderRadius: 9, background: "#fbfcff", padding: "9px 10px", cursor: "pointer" }}><div style={{ fontSize: 13, fontWeight: 700, color: "var(--nf-ink)" }}>{audit.title}</div><div style={{ marginTop: 4, fontSize: 11, color: "var(--nf-ink-3)" }}>{new Date(audit.plannedDate!).toLocaleDateString("es")} · {audit.type} · {audit.status}</div></button>)}</div></div>)}</div> : <EmptyOperational>Aún no hay fechas de planeamiento. Crea o edita auditorías para construir el cronograma.</EmptyOperational>}
        ...
        <div style={{ display: "grid", gap: 10, marginTop: 8 }}>
          {bulkRows.map((row, index) => <div key={row.id} style={{ border: "1px solid var(--nf-line)", borderRadius: 11, padding: 12, display: "grid", gap: 10 }}>
            ...
            <div className="nf-grid-2" style={{ gap: 10 }}>
              <Field label="Título"><input className="nf-app-input" style={inputStyle} value={row.title} onChange={(event) => updateBulkRow(row.id, "title", event.target.value)} required placeholder="Auditoría del proceso…" /></Field>
              <Field label="Tipo"><select className="nf-app-input" style={inputStyle} value={row.type} onChange={(event) => updateBulkRow(row.id, "type", event.target.value)}>{Object.values(AuditType).map((value) => <option key={value}>{value}</option>)}</select></Field>
              <Field label="Programa"><select className="nf-app-input" style={inputStyle} value={row.programId} onChange={(event) => updateBulkRow(row.id, "programId", event.target.value)}><option value="">Sin programa</option>{initial.programs.map((program) => <option key={program.id} value={program.id}>{program.year} · {program.title}</option>)}</select></Field>
              <Field label="Auditor interno"><select className="nf-app-input" style={inputStyle} value={row.auditorId} onChange={(event) => updateBulkRow(row.id, "auditorId", event.target.value)}><option value="">Sin asignar</option>{initial.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field>
              <Field label="Fecha de planeamiento"><input type="date" className="nf-app-input" style={inputStyle} value={row.plannedDate} onChange={(event) => updateBulkRow(row.id, "plannedDate", event.target.value)} /></Field>
              <Field label="Fecha de ejecución estimada"><input type="date" className="nf-app-input" style={inputStyle} value={row.scheduledDate} onChange={(event) => updateBulkRow(row.id, "scheduledDate", event.target.value)} /></Field>
              <Field label="Norma"><input className="nf-app-input" style={inputStyle} value={row.standardCode} onChange={(event) => updateBulkRow(row.id, "standardCode", event.target.value)} /></Field>
            </div>
          </div>)}
        </div>
    ...
    <FormModal open={creating || !!editing} title={editing ? "Editar auditoría" : "Nueva auditoría"} pending={isPending} error={error} onClose={() => { setCreating(false); setEditing(null); setError(""); }} onSubmit={submit}>
      ...
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Norma"><input name="standardCode" className="nf-app-input" style={inputStyle} defaultValue={row?.standardCode ?? "ISO 9001"} /></Field><Field label="Fecha de planeamiento"><input name="plannedDate" type="date" className="nf-app-input" style={inputStyle} defaultValue={row?.plannedDate?.slice(0, 10) ?? ""} /></Field></div>
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Proceso auditado"><select name="processId" className="nf-app-input" style={inputStyle} defaultValue={row?.processId ?? ""}><option value="">Sin proceso</option>{initial.processes.map((process) => <option key={process.id} value={process.id}>{process.code} · {process.name}</option>)}</select></Field><Field label="Fecha de ejecución estimada"><input name="scheduledDate" type="date" className="nf-app-input" style={inputStyle} defaultValue={row?.scheduledDate?.slice(0, 10) ?? ""} /></Field></div>
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Fecha de inicio"><input name="startDate" type="date" className="nf-app-input" style={inputStyle} defaultValue={row?.startDate?.slice(0, 10) ?? ""} /></Field><Field label="Fecha de fin"><input name="endDate" type="date" className="nf-app-input" style={inputStyle} defaultValue={row?.endDate?.slice(0, 10) ?? ""} /></Field></div>
      ...
      <Field label="Auditados"><select name="auditeeIds" multiple className="nf-app-input" style={{ ...inputStyle, minHeight: 88 }} defaultValue={row?.participants.map((participant) => participant.id) ?? []}>{initial.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field>
    ...
    <FormModal open={!!checklistAudit} title="Nueva pregunta de checklist" pending={isPending} error={error} onClose={() => setChecklistAudit(null)} onSubmit={submitChecklist}><Field label="Cláusula ISO"><select name="clauseId" className="nf-app-input" style={inputStyle} defaultValue=""><option value="">Seleccionar cláusula…</option>{initial.clauses.map((clause) => <option key={clause.id} value={clause.id}>{clause.standardCode} · {clause.code} · {clause.title}</option>)}</select></Field><Field label="Código de cláusula (opcional)"><input name="clauseCode" className="nf-app-input" style={inputStyle} /></Field><Field label="Pregunta"><textarea name="question" required rows={3} className="nf-app-input" style={inputStyle} /></Field><Field label="Resultado esperado"><textarea name="expected" rows={2} className="nf-app-input" style={inputStyle} /></Field></FormModal>
