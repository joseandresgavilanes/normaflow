...
function Stat({ label, children }: { label: string; children: ReactNode }) {
  return <div style={{ border: "1px solid var(--nf-line)", borderRadius: 9, padding: 9 }}><div style={{ fontSize: 10, color: "var(--nf-ink-3)", textTransform: "uppercase", fontWeight: 800 }}>{label}</div><div style={{ marginTop: 3, fontWeight: 800 }}>{children}</div></div>;
}
...
export function ChangesLiveClient({ initial }: { initial: ChangesPayload }) {
  ...
  return <div>
    ...
    {initial.changes.length === 0 ? <EmptyOperational>No hay solicitudes de cambio. Crea la primera para iniciar su evaluación.</EmptyOperational> : <OperationalGrid>{initial.changes.map((change) => <OperationalCard key={change.id} onClick={() => setDetail(change)}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div><div style={{ fontFamily: "ui-monospace, monospace", fontSize: 12, color: "#123C66", fontWeight: 850 }}>{change.code}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{change.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{change.category} · {change.changeType}</div></div><Badge status={change.status} /></div>
    ...
    <FormModal open={creating || !!editing} title={editing ? "Editar solicitud" : "Nueva solicitud de cambio"} pending={isPending} error={error} onClose={() => { setCreating(false); setEditing(null); setError(""); }} onSubmit={submit}>
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Código (opcional)"><input name="code" className="nf-app-input" style={inputStyle} defaultValue={row?.code ?? ""} /></Field><Field label="Impacto"><select name="impact" className="nf-app-input" style={inputStyle} defaultValue={row?.impact ?? ChangeImpact.MEDIUM}>{Object.values(ChangeImpact).map((value) => <option key={value}>{value}</option>)}</select></Field></div>
      <Field label="Título"><input name="title" required className="nf-app-input" style={inputStyle} defaultValue={row?.title ?? ""} /></Field>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Categoría">
          <select name="category" required className="nf-app-input" style={inputStyle} defaultValue={row?.category ?? DEFAULT_CHANGE_CATEGORY}>
        ...
        <Field label="Tipo de cambio">
          <select name="changeType" required className="nf-app-input" style={inputStyle} defaultValue={row?.changeType ?? DEFAULT_CHANGE_TYPE}>
      ...
      <Field label="Justificación"><textarea name="reason" required rows={3} className="nf-app-input" style={inputStyle} defaultValue={row?.reason ?? ""} /></Field>
      <Field label="Áreas afectadas (separadas por comas)"><input name="affectedAreas" className="nf-app-input" style={inputStyle} defaultValue={row?.affectedAreas.join(", ") ?? ""} /></Field>
      {initial.nonconformities.length > 0 && <Field label="No conformidad origen"><select name="nonconformityId" className="nf-app-input" style={inputStyle} defaultValue={row?.nonconformityId ?? ""}><option value="">Sin vínculo</option>{initial.nonconformities.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></Field>}
...
export function IntegrationsLiveClient({ initial }: { initial: IntegrationsPayload }) {
  ...
  return <div><OperationalHeader title="Integraciones" subtitle={`${initial.integrations.length} configuraciones registradas; no hay conexiones simuladas`} canCreate={initial.access.canManage} actionLabel="Registrar integración" onCreate={() => { setError(""); setCreating(true); }} /><OperationalMessages error={error} success={success} />
    {initial.integrations.length === 0 ? <EmptyOperational>No hay integraciones configuradas. Registrar una no ejecuta OAuth ni guarda secretos: documenta una conexión real que debe implementar su adaptador.</EmptyOperational> : <OperationalGrid>{initial.integrations.map((integration) => <OperationalCard key={integration.id} onClick={() => setDetail(integration)}><div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><div style={{ fontSize: 11, fontWeight: 800, color: "var(--nf-ink-3)", textTransform: "uppercase" }}>{integration.category} · {integration.provider}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{integration.name}</h3><code style={{ fontSize: 11 }}>{integration.key}</code></div><Badge status={integration.status} /></div><p style={{ fontSize: 13, lineHeight: 1.5, color: "var(--nf-ink-2)" }}>{integration.description ?? "Sin descripción"}</p><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>Última sincronización real: {integration.lastSyncAt ? new Date(integration.lastSyncAt).toLocaleString("es") : "nunca"}</div><CardActions canUpdate={initial.access.canManage} canDelete={initial.access.canManage} pending={isPending} onEdit={() => setEditing(integration)} onDelete={() => remove(integration)} /></OperationalCard>)}</OperationalGrid>}
