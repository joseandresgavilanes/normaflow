...
export function ChangesLiveClient({ initial }: { initial: ChangesPayload }) {
  ...
  return <div>
    ...
    {initial.changes.length === 0 ? <EmptyOperational>No hay solicitudes de cambio. Crea la primera para iniciar su evaluación.</EmptyOperational> : <OperationalGrid>{initial.changes.map((change) => <OperationalCard key={change.id} onClick={() => setDetail(change)}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div><div style={{ fontFamily: "ui-monospace, monospace", fontSize: 12, color: "var(--nf-primary-active)", fontWeight: 850 }}>{change.code}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{change.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{change.category} · {change.changeType}</div></div><Badge status={change.status} /></div>
