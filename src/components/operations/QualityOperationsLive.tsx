...
function RequirementsTab({ initial, run, isPending, setError, creating, editing, setEditing, openCreate, closeForm, processOptions }: {
  ...
  return <>
    ...
    <FormModal open={creating || !!editing} title={editing ? "Editar requisito" : "Nuevo requisito del cliente"} pending={isPending} error="" onClose={closeForm} onSubmit={submit}>
      <Field label="Título"><input name="title" required className="nf-app-input" style={inputStyle} defaultValue={editing?.title ?? ""} /></Field>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Fuente"><input name="source" className="nf-app-input" style={inputStyle} placeholder="Cliente, contrato, licitación…" defaultValue={editing?.source ?? ""} /></Field>
        <Field label="Proceso"><select name="processId" className="nf-app-input" style={inputStyle} defaultValue={editing?.processId ?? ""}><option value="">Sin asignar</option>{processOptions.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
      </div>
      ...
      <Field label="Estado"><select name="status" className="nf-app-input" style={inputStyle} defaultValue={editing?.status ?? "OPEN"}><option value="OPEN">Abierto</option><option value="REVIEWED">Revisado</option><option value="MET">Cumplido</option></select></Field>
    </FormModal>
  </>;
}
...
function PropertyTab({ initial, run, isPending, setError, creating, openCreate, closeForm, processOptions, memberOptions }: {
  ...
  return <>
    ...
    <FormModal open={creating} title="Nueva propiedad del cliente" pending={isPending} error="" onClose={closeForm} onSubmit={submit}>
      <Field label="Descripción del bien"><input name="description" required className="nf-app-input" style={inputStyle} /></Field>
      <Field label="Cliente"><input name="customerName" required className="nf-app-input" style={inputStyle} /></Field>
      <Field label="Condición al recibir"><textarea name="conditionOnReceipt" rows={2} className="nf-app-input" style={inputStyle} /></Field>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        ...
        <Field label="Proceso"><select name="processId" className="nf-app-input" style={inputStyle}><option value="">Sin asignar</option>{processOptions.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
      </div>
    </FormModal>
    <Modal open={!!reporting} onClose={() => setReporting(null)} title="Reportar incidente" width={520}>{reporting && <form onSubmit={(e) => {
      ...
      <Field label="Descripción del incidente"><textarea name="incidentNote" required rows={4} className="nf-app-input" style={inputStyle} placeholder="Qué ocurrió, cuándo se detectó y qué se comunicó al cliente." /></Field>
...
function PreservationTab({ initial, run, isPending, setError, creating, editing, setEditing, openCreate, closeForm, processOptions, memberOptions }: {
  ...
  return <>
    ...
    <FormModal open={creating || !!editing} title={editing ? "Editar registro" : "Nuevo registro de preservación"} pending={isPending} error="" onClose={closeForm} onSubmit={submit}>
      <Field label="Elemento / salida"><input name="itemDescription" required className="nf-app-input" style={inputStyle} defaultValue={editing?.itemDescription ?? ""} /></Field>
      <Field label="Instrucciones de manipulación"><textarea name="handlingInstructions" rows={2} className="nf-app-input" style={inputStyle} defaultValue={editing?.handlingInstructions ?? ""} /></Field>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Condiciones de almacenamiento"><textarea name="storageConditions" rows={2} className="nf-app-input" style={inputStyle} defaultValue={editing?.storageConditions ?? ""} /></Field>
        <Field label="Nota de embalaje/transporte"><textarea name="packagingNote" rows={2} className="nf-app-input" style={inputStyle} defaultValue={editing?.packagingNote ?? ""} /></Field>
      </div>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Responsable"><select name="responsibleId" className="nf-app-input" style={inputStyle} defaultValue={editing?.responsibleId ?? ""}><option value="">Sin asignar</option>{memberOptions.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></Field>
        <Field label="Proceso"><select name="processId" className="nf-app-input" style={inputStyle} defaultValue={editing?.processId ?? ""}><option value="">Sin asignar</option>{processOptions.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
      </div>
      <Field label="Estado"><select name="status" className="nf-app-input" style={inputStyle} defaultValue={editing?.status ?? "UNDER_REVIEW"}><option value="UNDER_REVIEW">En revisión</option><option value="COMPLIANT">Conforme</option><option value="NON_COMPLIANT">No conforme</option></select></Field>
    </FormModal>
  </>;
}
...
function FeedbackTab({ initial, run, isPending, setError, creating, openCreate, closeForm }: {
  ...
  return <>
    ...
    <FormModal open={creating} title="Nueva retroalimentación de cliente" pending={isPending} error="" onClose={closeForm} onSubmit={submit}>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Cliente"><input name="customerName" className="nf-app-input" style={inputStyle} /></Field>
        <Field label="Canal"><select name="channel" className="nf-app-input" style={inputStyle}><option value="SURVEY">Encuesta</option><option value="COMPLAINT">Queja</option><option value="COMPLIMENT">Felicitación</option><option value="REVIEW">Reseña</option><option value="INTERVIEW">Entrevista</option><option value="OTHER">Otro</option></select></Field>
      </div>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Puntaje (0-100)"><input name="score" type="number" min={0} max={100} className="nf-app-input" style={inputStyle} /></Field>
        <Field label="Fecha recibida"><input name="receivedAt" type="date" className="nf-app-input" style={inputStyle} /></Field>
      </div>
      <Field label="Comentario"><textarea name="comment" rows={3} className="nf-app-input" style={inputStyle} /></Field>
      <Field label="CAPA vinculada (si aplica)"><select name="linkedCapaId" className="nf-app-input" style={inputStyle}><option value="">Ninguna</option>{initial.capas.map((c) => <option key={c.id} value={c.id}>{c.code} · {c.title}</option>)}</select></Field>
    </FormModal>
  </>;
}
...
function CommunicationTab({ initial, run, isPending, setError, creating, editing, setEditing, openCreate, closeForm, memberOptions }: {
  ...
  return <>
    ...
    <FormModal open={creating || !!editing} title={editing ? "Editar comunicación" : "Nueva comunicación"} pending={isPending} error="" onClose={closeForm} onSubmit={submit}>
      <Field label="Asunto"><input name="subject" required className="nf-app-input" style={inputStyle} defaultValue={editing?.subject ?? ""} /></Field>
      <Field label="Contenido"><textarea name="content" rows={3} className="nf-app-input" style={inputStyle} defaultValue={editing?.content ?? ""} /></Field>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Dirección"><select name="direction" className="nf-app-input" style={inputStyle} defaultValue={editing?.direction ?? "INTERNAL"}><option value="INTERNAL">Interna</option><option value="EXTERNAL">Externa</option></select></Field>
        <Field label="Audiencia"><input name="audience" className="nf-app-input" style={inputStyle} placeholder="Empleados, clientes, autoridad…" defaultValue={editing?.audience ?? ""} /></Field>
      </div>
      <div className="nf-grid-2" style={{ gap: 12 }}>
        <Field label="Canal"><input name="channel" className="nf-app-input" style={inputStyle} placeholder="Email, reunión, boletín…" defaultValue={editing?.channel ?? ""} /></Field>
        <Field label="Fecha"><input name="communicatedAt" type="date" className="nf-app-input" style={inputStyle} defaultValue={editing?.communicatedAt?.slice(0, 10) ?? ""} /></Field>
      </div>
      <Field label="Quién comunica"><select name="communicatedById" className="nf-app-input" style={inputStyle} defaultValue={editing?.communicatedById ?? ""}><option value="">Sin asignar</option>{memberOptions.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></Field>
    </FormModal>
  </>;
}
