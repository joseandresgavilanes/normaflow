...
export function RisksLiveClient({ initial }: { initial: RisksPayload }) {
  ...
  return <div>
    ...
    {initial.risks.length === 0 ? <EmptyOperational>No hay riesgos registrados.</EmptyOperational> : <OperationalGrid>{initial.risks.map((row) => <OperationalCard key={row.id} onClick={() => setDetail(row)}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div style={{ minWidth: 0 }}><div style={{ fontSize: 11, color: "var(--nf-ink-2, #223648)", textTransform: "none", fontWeight: 600 }}>{row.category}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{row.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3, #314456)" }}>{row.processCode ? `${row.processCode} · ${row.processName}` : "Sin proceso"}</div></div><div style={{ width: 48, height: 48, borderRadius: 13, display: "grid", placeItems: "center", fontSize: 20, fontWeight: 900, color: row.score >= 15 ? "var(--nf-danger-text)" : row.score >= 8 ? "var(--nf-warning-text)" : "#17633b", background: row.score >= 15 ? "#fff0ef" : row.score >= 8 ? "#fff8e6" : "#edf9f2" }}>{row.score}</div></div>
