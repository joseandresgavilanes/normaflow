  ...
  return (
    <div>
            ...
            <OperationalCard key={row.id} onClick={() => setDetail(row)}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: "ui-monospace, monospace", color: "var(--nf-primary-active)", fontSize: 12, fontWeight: 600 }}>{row.code ?? "SIN CÓDIGO"}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 7, marginTop: 13 }}>
                {[["Docs", row.counts.documents], ["Riesgos", row.counts.risks], ["KPIs", row.counts.indicators], ["Formación", row.counts.trainingAssignments]].map(([label, value]) => (
                  <div key={String(label)} style={{ background: "var(--nf-app-surface-1)", borderRadius: 9, padding: "8px 4px", textAlign: "center" }}><strong style={{ display: "block", color: "var(--nf-primary-active)" }}>{value}</strong><span style={{ fontSize: 9, color: "var(--nf-ink-3)", textTransform: "none" }}>{label}</span></div>
                ))}
              </div>
      ...
      <FormModal open={creating || !!editing} title={editing ? "Editar proceso" : "Nuevo proceso"} pending={isPending} error={error} onClose={() => { setCreating(false); setEditing(null); setError(""); }} onSubmit={submit}>
        ...
        <div className="nf-grid-2" style={{ gap: 12 }}>
          <Field label="Tipo"><select aria-label="Tipo" name="type" className="nf-app-input" style={inputStyle} defaultValue={formRow?.type ?? "core"}><option value="core">Core</option><option value="support">Soporte</option><option value="strategic">Estratégico</option></select></Field>
          <Field label="Responsable"><select aria-label="Responsable" name="ownerId" className="nf-app-input" style={inputStyle} defaultValue={formRow?.ownerId ?? ""}><option value="">Sin asignar</option>{initial.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field>
        </div>
...
export function RisksLiveClient({ initial }: { initial: RisksPayload }) {
  ...
  return <div>
    ...
    {initial.risks.length === 0 ? <EmptyOperational>No hay riesgos registrados.</EmptyOperational> : <OperationalGrid>{initial.risks.map((row) => <OperationalCard key={row.id} onClick={() => setDetail(row)}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div style={{ minWidth: 0 }}><div style={{ fontSize: 11, color: "var(--nf-ink-2)", textTransform: "none", fontWeight: 600 }}>{row.category}</div><h3 style={{ margin: "6px 0", fontSize: 17 }}>{row.title}</h3><div style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{row.processCode ? `${row.processCode} · ${row.processName}` : "Sin proceso"}</div></div><div style={{ width: 48, height: 48, borderRadius: 13, display: "grid", placeItems: "center", fontSize: 20, fontWeight: 900, color: row.score >= 15 ? "var(--nf-danger-text)" : row.score >= 8 ? "var(--nf-warning-text)" : "var(--nf-success-text)", background: row.score >= 15 ? "var(--nf-danger-subtle)" : row.score >= 8 ? "var(--nf-warning-subtle)" : "var(--nf-success-subtle)" }}>{row.score}</div></div>
      <div style={{ display: "flex", gap: 7, marginTop: 12, flexWrap: "wrap" }}><Badge status={row.status} /><Badge status="ACTIVE" label={row.treatment} /><span style={{ fontSize: 12, color: "var(--nf-ink-3)", fontWeight: 600 }}>{row.ownerName ?? "Sin responsable"}</span></div>
      <CardActions canUpdate={initial.access.canUpdate} canDelete={initial.access.canDelete} pending={isPending} onEdit={() => { setError(""); setEditing(row); }} onDelete={() => remove(row)} />
    ...
    <FormModal open={creating || !!editing} title={editing ? "Editar riesgo" : "Nuevo riesgo"} pending={isPending} error={error} onClose={() => { setCreating(false); setEditing(null); setError(""); }} onSubmit={submit}>
      ...
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Categoría"><select aria-label="Categoría" name="category" className="nf-app-input" style={inputStyle} defaultValue={formRow?.category ?? DEFAULT_RISK_CATEGORY} required>{riskCategoryOptions(formRow?.category).map((option) => <option key={option} value={option}>{option}</option>)}</select></Field><Field label="Proceso"><select aria-label="Proceso" name="processId" className="nf-app-input" style={inputStyle} defaultValue={formRow?.processId ?? ""}><option value="">Sin proceso</option>{initial.processes.map((process) => <option key={process.id} value={process.id}>{process.code ?? "PROC"} · {process.name}</option>)}</select></Field></div>
      ...
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Estado"><div className="nf-app-input" style={{ ...inputStyle, color: "var(--nf-ink-2)" }}>{formRow?.status ?? RiskStatus.IDENTIFIED}</div><input type="hidden" name="status" value={formRow?.status ?? RiskStatus.IDENTIFIED} /></Field><Field label="Tratamiento"><select aria-label="Tratamiento" name="treatment" className="nf-app-input" style={inputStyle} defaultValue={formRow?.treatment ?? RiskTreatment.MITIGATE}>{Object.values(RiskTreatment).map((value) => <option key={value}>{value}</option>)}</select></Field></div>
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Responsable"><select aria-label="Responsable" name="ownerId" className="nf-app-input" style={inputStyle} defaultValue={formRow?.ownerId ?? ""}><option value="">Sin asignar</option>{initial.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field><Field label="Fecha objetivo"><input aria-label="Fecha de vencimiento" name="dueDate" type="date" className="nf-app-input" style={inputStyle} defaultValue={formRow?.dueDate?.slice(0, 10) ?? ""} /></Field></div>
    ...
    <FormModal open={!!controlRisk} title={editingControl ? "Editar control" : `Nuevo control · ${controlRisk?.title ?? ""}`} pending={isPending} error={error} onClose={() => { setControlRisk(null); setEditingControl(null); setError(""); }} onSubmit={submitControl}>
      ...
      <div className="nf-grid-2" style={{ gap: 12 }}><Field label="Tipo"><select aria-label="Tipo" name="type" className="nf-app-input" style={inputStyle} defaultValue={editingControl?.type ?? ControlType.PREVENTIVE}>{Object.values(ControlType).map((value) => <option key={value}>{value}</option>)}</select></Field><Field label="Estado"><div className="nf-app-input" style={{ ...inputStyle, color: "var(--nf-ink-2)" }}>{editingControl?.status ?? ControlStatus.PLANNED}</div><input type="hidden" name="status" value={editingControl?.status ?? ControlStatus.PLANNED} /></Field></div>
      <Field label="Responsable"><select aria-label="Responsable" name="ownerId" className="nf-app-input" style={inputStyle} defaultValue={editingControl?.ownerId ?? ""}><option value="">Sin asignar</option>{initial.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field>
