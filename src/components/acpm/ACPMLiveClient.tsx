...
export function ACPMLiveClient({ initial }: { initial: CAPAPayload }) {
  ...
  return <div>
    ...
    <div className="nf-metric-strip"><Metric label="Abiertas" value={open.length} /><Metric label="En implementación" value={initial.capas.filter((row) => row.stage === "IMPLEMENTATION").length} color="var(--nf-primary-active)" /><Metric label="Por verificar" value={initial.capas.filter((row) => row.stage === "VERIFICATION").length} color="var(--nf-warning-text)" /><Metric label="Vencidas" value={overdue} color="var(--nf-danger-text)" /><Metric label="Cerradas" value={initial.capas.filter((row) => row.stage === "CLOSED").length} color="var(--nf-success-text)" /></div>
...
function CreateModal({ initial, open, pending, onClose, onSubmit }: { initial: CAPAPayload; open: boolean; pending: boolean; onClose: () => void; onSubmit: (input: CreateCAPAInput) => void }) {
  return <Modal open={open} onClose={onClose} title="Registrar no conformidad" width={760}><form className="nf-modal-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const evidenceFile = form.get("evidenceFile"); onSubmit({ title: String(form.get("title") ?? ""), description: String(form.get("description") ?? ""), origin: String(form.get("origin") ?? "OTHER") as Origin, standardCode: String(form.get("standardCode") ?? "") || undefined, clauseId: String(form.get("clauseId") ?? "") || undefined, processId: String(form.get("processId") ?? "") || undefined, severity: String(form.get("severity") ?? "MINOR") as Severity, priority: String(form.get("priority") ?? "MEDIUM") as CreateCAPAInput["priority"], ownerId: String(form.get("ownerId") ?? "") || undefined, dueDate: String(form.get("dueDate") ?? "") || undefined, evidenceTitle: String(form.get("evidenceTitle") ?? "") || undefined, evidenceFile: evidenceFile instanceof File && evidenceFile.size ? evidenceFile : undefined }); }}>
    ...
    <div className="nf-form-grid-3"><Field label="Origen *"><Picker aria-label="Origen" name="origin" defaultValue="OTHER" className={NF_INPUT_CLASS} style={modalInputStyle}>{ORIGINS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Picker></Field><Field label="Severidad *"><Picker aria-label="Severidad" name="severity" defaultValue="MINOR" className={NF_INPUT_CLASS} style={modalInputStyle}>{SEVERITIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Picker></Field><Field label="Prioridad *"><Picker aria-label="Prioridad" name="priority" defaultValue="MEDIUM" className={NF_INPUT_CLASS} style={modalInputStyle}><option value="CRITICAL">Crítica</option><option value="HIGH">Alta</option><option value="MEDIUM">Media</option><option value="LOW">Baja</option></Picker></Field></div>
    <div className="nf-form-grid-3"><ClausePicker clauses={initial.clauses} standardName="standardCode" labelClause="Cláusula ISO" inputClassName={NF_INPUT_CLASS} inputStyle={modalInputStyle} /><Field label="Proceso"><Picker aria-label="Proceso" name="processId" className={NF_INPUT_CLASS} style={modalInputStyle}><option value="">—</option>{initial.processes.map((item) => <option key={item.id} value={item.id}>{item.code ? `${item.code} · ` : ""}{item.name}</option>)}</Picker></Field></div>
    <div className="nf-form-grid-3"><Field label="Responsable"><PersonPicker name="ownerId" people={initial.members} placeholder="Sin asignar" ariaLabel="Responsable" style={modalInputStyle} /></Field><Field label="Fecha límite"><DateField aria-label="Fecha de vencimiento" name="dueDate" className={NF_INPUT_CLASS} style={modalInputStyle} /></Field><div /></div><div className="nf-form-grid-2"><FileImportArea name="evidenceFile" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.csv,.txt,.md,image/*" maxSizeMB={50} label="Evidencia inicial (opcional)" compact /><Field label="Título de evidencia"><input aria-label="Acta, foto, informe" name="evidenceTitle" placeholder="Acta, foto, informe…" className={NF_INPUT_CLASS} style={modalInputStyle} /></Field></div>
...
function EvidenceForm({ row, pending, onRun }: { row: Row; pending: boolean; onRun: (operation: () => Promise<unknown>, message: string) => void }) { const [kind, setKind] = useState<EvidenceKind>(row.stage === "VERIFICATION" ? "EFFECTIVENESS" : row.stage === "IMPLEMENTATION" ? "IMPLEMENTATION" : "NONCONFORMITY"); return <form onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const file = form.get("file"); if (!(file instanceof File) || !file.size) return; onRun(() => uploadCAPAEvidence({ capaId: row.id, kind, title: String(form.get("title") ?? ""), file }), "Evidencia adjuntada."); }} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 8, alignItems: "end" }}><Field label="Tipo"><Picker aria-label="Tipo" value={kind} onChange={(event) => setKind(event.target.value as EvidenceKind)} className={NF_INPUT_CLASS} style={modalInputStyle}><option value="NONCONFORMITY">No conformidad</option><option value="IMPLEMENTATION">Implementación</option><option value="EFFECTIVENESS">Eficacia</option></Picker></Field><Field label="Título"><input aria-label="Título" name="title" required className={NF_INPUT_CLASS} style={modalInputStyle} /></Field><FileImportArea name="file" required accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.csv,.txt,.md,image/*" maxSizeMB={50} label="Archivo" compact /><button type="submit" className="nf-app-btn-ghost" disabled={pending}>{pending ? <Loader2 size={14} className="nf-icon-spin" /> : <Paperclip size={14} />} Adjuntar</button></form>; }
...
function Filter({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: { value: string; label: string }[] }) { return <Picker aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className={NF_INPUT_CLASS} style={{ width: "auto", minWidth: 150 }}>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Picker>; }
