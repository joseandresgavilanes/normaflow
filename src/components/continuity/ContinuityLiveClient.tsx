  ...
  return <div className="nf-iso-module">
    ...
    <Card>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
        ...
        {initial.canExport && <><select className="nf-app-input" aria-label="Informe de esta sección" value={reportTypeByTab[tab] ?? reportOptions[0]?.value} onChange={(e) => setReportTypeByTab((current) => ({ ...current, [tab]: e.target.value }))} style={{ maxWidth: 220 }}>{reportOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><button type="button" className="nf-app-btn-ghost" disabled={exporting} onClick={() => void exportReport("EXCEL")}><Download size={14} />Excel</button><button type="button" className="nf-app-btn-ghost" disabled={exporting} onClick={() => void exportReport("PDF")}><Download size={14} />PDF</button></>}
      </div>
      ...
      {tab === "panel" && <div className="nf-iso-dashboard-grid" style={{ display: "grid", gap: 16 }}>
        ...
        <div className="nf-iso-dashboard-card nf-iso-dashboard-card--wide">
          <div className="nf-iso-dashboard-card-head"><div><h3 className="nf-iso-dashboard-card-title"><FileText size={16} aria-hidden />Planes de continuidad</h3><p className="nf-iso-dashboard-card-subtitle">Estado, objetivos de recuperación y cobertura de procesos críticos.</p></div></div>
      ...
      {tab === "plans" && <div style={{ display: "grid", gap: 12 }}>
        <ContinuityFilterBar query={planQuery} onQueryChange={setPlanQuery} placeholder="Buscar por código, título o alcance…"><select className="nf-app-input" aria-label="Estado del plan" value={planStatus} onChange={(e) => setPlanStatus(e.target.value)}><option value="ALL">Todos los estados</option>{Object.entries(PLAN_STATUS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select className="nf-app-input" aria-label="Tipo de plan" value={planKind} onChange={(e) => setPlanKind(e.target.value)}><option value="ALL">BCP y DRP</option><option value="BCP">Solo BCP</option><option value="DRP">Solo DRP</option></select></ContinuityFilterBar>
...
function PlanCard({ bcp, initial, pending, onRun }: { bcp: Bcp; initial: ContinuityPayload; pending: boolean; onRun: ReturnType<typeof useServerAction>["run"] }) {
  ...
  return <div className="nf-continuity-plan-card">
    ...
    <Modal open={showDeactivate} onClose={() => setShowDeactivate(false)} title="Cerrar activación" width={620}>
      <div className="nf-continuity-detail-form-stack"><label>Resultado<textarea className="nf-app-input" rows={2} maxLength={4000} value={deactivation.outcome} onChange={(e) => setDeactivation((f) => ({ ...f, outcome: e.target.value }))} /></label><label>Lecciones aprendidas<textarea className="nf-app-input" rows={3} maxLength={8000} value={deactivation.lessonsLearned} onChange={(e) => setDeactivation((f) => ({ ...f, lessonsLearned: e.target.value }))} /></label><label>Evidencia<select className="nf-app-input" value={deactivation.evidenceId} onChange={(e) => setDeactivation((f) => ({ ...f, evidenceId: e.target.value }))}><option value="">Sin evidencia</option>{initial.evidenceOptions.map((ev) => <option key={ev.id} value={ev.id}>{ev.title}</option>)}</select></label><div className="nf-modal-actions"><button type="button" className="nf-app-btn-ghost" onClick={() => setShowDeactivate(false)}>Cancelar</button><button type="button" className="nf-app-btn-danger" disabled={pending || !activeActivation} onClick={() => activeActivation && onRun(() => deactivatePlan({ id: activeActivation.id, outcome: optionalText(deactivation.outcome, "El resultado", 4000) ?? null, lessonsLearned: optionalText(deactivation.lessonsLearned, "Las lecciones aprendidas", 8000) ?? null, evidenceId: assertOptions(deactivation.evidenceId, initial.evidenceOptions.map((ev) => ev.id), "La evidencia") || null }), { onSuccess: () => { setDeactivation({ outcome: "", lessonsLearned: "", evidenceId: "" }); setShowDeactivate(false); }, successMessage: "Plan desactivado." })}>Cerrar activación</button></div></div>
    </Modal>
    ...
    <div>
      ...
      <div className="nf-data-table-wrap"><table className="nf-data-table" style={{ minWidth: 820 }}>
        ...
        <tbody>{visibleBias.map((b) => <tr key={b.id}>
          ...
          <td>{p.canUpdate && b.status !== "APPROVED" && <button type="button" className="nf-app-btn-ghost" onClick={() => { setEditingBia(editingBia === b.id ? null : b.id); setBiaEdit({ title: b.title, scope: b.scope ?? "", methodology: b.methodology ?? "", version: b.version, performedAt: b.performedAt ?? "", nextReviewDate: b.nextReviewDate ?? "", ownerId: b.owner?.id ?? "", evidenceId: b.evidenceId ?? "" }); }}>Editar</button>} {p.canApprove && b.status !== "APPROVED" && <button type="button" className="nf-app-btn-ghost" disabled={pending} onClick={() => onRun(() => approveBia({ id: b.id }), { successMessage: "BIA aprobado." })}>Aprobar</button>}</td>
        </tr>)}</tbody></table>
      ...
      <Modal open={Boolean(editingBiaRow)} onClose={() => setEditingBia(null)} title={`Editar BIA · ${editingBiaRow?.code ?? ""}`} width={700}>
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
            ...
            <label>Realizado<input className="nf-app-input" type="date" value={biaEdit.performedAt} onChange={(e) => setBiaEdit((f) => ({ ...f, performedAt: e.target.value }))} /></label>
            <label>Próxima revisión<input className="nf-app-input" type="date" value={biaEdit.nextReviewDate} onChange={(e) => setBiaEdit((f) => ({ ...f, nextReviewDate: e.target.value }))} /></label>
          </div>
          <label>Propietario<select className="nf-app-input" value={biaEdit.ownerId} onChange={(e) => setBiaEdit((f) => ({ ...f, ownerId: e.target.value }))}><option value="">Sin asignar</option>{p.members.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label>
          <label>Evidencia<select className="nf-app-input" value={biaEdit.evidenceId} onChange={(e) => setBiaEdit((f) => ({ ...f, evidenceId: e.target.value }))}><option value="">Sin evidencia</option>{p.evidenceOptions.map((e) => <option key={e.id} value={e.id}>{e.title}</option>)}</select></label>
      ...
      <div className="nf-data-table-wrap"><table className="nf-data-table" style={{ minWidth: 1000 }}>
        ...
        <tbody>{visibleActivities.map((a) => <Fragment key={a.id}>
          <tr>
            ...
            <td>{p.canUpdate && <button type="button" className="nf-app-btn-ghost" onClick={() => { setEditingActivity(editingActivity === a.id ? null : a.id); setEditValues({ mtpdMinutes: a.mtpdMinutes != null ? String(a.mtpdMinutes) : "", rtoMinutes: a.rtoMinutes != null ? String(a.rtoMinutes) : "", rpoMinutes: a.rpoMinutes != null ? String(a.rpoMinutes) : "", minimumServiceLevel: a.minimumServiceLevel ?? "" }); }}>Editar</button>}</td>
          </tr>
    ...
    <div>
      ...
      <div className="nf-data-table-wrap"><table className="nf-data-table" style={{ minWidth: 820 }}>
        ...
        <tbody>{visiblePriorities.map((x) => <tr key={x.id}>
          ...
          <td>{x.revenueShare != null ? `${x.revenueShare}%` : "—"}</td><td>{x.customersAffected ?? "—"}</td><td>{p.canUpdate && <button type="button" className="nf-app-btn-ghost" onClick={() => { setEditingPriority(editingPriority === x.id ? null : x.id); setPriorityEdit({ name: x.name, criticality: x.criticality, mtpdMinutes: x.mtpdMinutes != null ? String(x.mtpdMinutes) : "", rtoMinutes: x.rtoMinutes != null ? String(x.rtoMinutes) : "", minimumServiceLevel: x.minimumServiceLevel ?? "", revenueShare: x.revenueShare != null ? String(x.revenueShare) : "", customersAffected: x.customersAffected != null ? String(x.customersAffected) : "", description: x.description ?? "", notes: x.notes ?? "" }); }}>Editar</button>}</td>
        </tr>)}</tbody></table>
        <Modal open={Boolean(editingPriorityRow)} onClose={() => setEditingPriority(null)} title={`Editar prioridad · ${editingPriorityRow?.code ?? ""}`} width={680}>
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}><label>Criticidad<select className="nf-app-input" value={priorityEdit.criticality} onChange={(e) => setPriorityEdit((f) => ({ ...f, criticality: e.target.value }))}>{Object.entries(CRITICALITY).map(([v, c]) => <option key={v} value={v}>{c.label}</option>)}</select></label><label>Nivel mínimo<input className="nf-app-input" value={priorityEdit.minimumServiceLevel} onChange={(e) => setPriorityEdit((f) => ({ ...f, minimumServiceLevel: e.target.value }))} /></label></div>
...
function DependenciesTab({ p, pending, onRun }: { p: ContinuityPayload; pending: boolean; onRun: ReturnType<typeof useServerAction>["run"] }) {
  ...
  return <div style={{ display: "grid", gap: 16 }}>
    <ContinuityFilterBar query={filterQuery} onQueryChange={setFilterQuery} placeholder="Buscar dependencia o recurso…"><select className="nf-app-input" aria-label="Tipo de dependencia o recurso" value={filterType} onChange={(e) => setFilterType(e.target.value)}><option value="ALL">Todos los tipos</option>{Array.from(new Set([...DEP_TYPE_OPTIONS, ...RES_TYPE_OPTIONS])).map((value) => <option key={value} value={value}>{DEP_TYPE[value] ?? value}</option>)}</select><select className="nf-app-input" aria-label="Puntos únicos de fallo" value={filterRisk} onChange={(e) => setFilterRisk(e.target.value)}><option value="ALL">Todos los riesgos</option><option value="SPOF">Solo puntos únicos de fallo</option><option value="NO_SPOF">Sin punto único de fallo</option></select></ContinuityFilterBar>
    <div>
      <IsoSectionHeader
        icon={Link2}
      ...
      <div className="nf-data-table-wrap"><table className="nf-data-table" style={{ minWidth: 960 }}>
        ...
        <Modal open={Boolean(editingDependencyRow)} onClose={() => setEditingDependency(null)} title={`Editar dependencia · ${editingDependencyRow?.name ?? ""}`} width={680}>
          <div style={{ display: "grid", gap: 12 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}><label>Tipo<select className="nf-app-input" value={dependencyEdit.type} onChange={(e) => setDependencyEdit((f) => ({ ...f, type: e.target.value }))}>{DEP_TYPE_OPTIONS.map((v) => <option key={v} value={v}>{DEP_TYPE[v] ?? v}</option>)}</select></label><label>Criticidad<select className="nf-app-input" value={dependencyEdit.criticality} onChange={(e) => setDependencyEdit((f) => ({ ...f, criticality: e.target.value }))}>{Object.entries(CRITICALITY).map(([v, c]) => <option key={v} value={v}>{c.label}</option>)}</select></label></div>
    ...
    <div>
      ...
      <div className="nf-data-table-wrap"><table className="nf-data-table" style={{ minWidth: 900 }}>
        ...
        <Modal open={Boolean(editingResourceRow)} onClose={() => setEditingResource(null)} title={`Editar recurso · ${editingResourceRow?.name ?? ""}`} width={680}>
          <div style={{ display: "grid", gap: 12 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}><label>Tipo<select className="nf-app-input" value={resourceEdit.type} onChange={(e) => setResourceEdit((f) => ({ ...f, type: e.target.value }))}>{RES_TYPE_OPTIONS.map((v) => <option key={v} value={v}>{DEP_TYPE[v] ?? v}</option>)}</select></label><label>Unidad<input className="nf-app-input" value={resourceEdit.unit} onChange={(e) => setResourceEdit((f) => ({ ...f, unit: e.target.value }))} /></label></div>
...
function StrategiesTab({ p, pending, onRun }: { p: ContinuityPayload; pending: boolean; onRun: ReturnType<typeof useServerAction>["run"] }) {
  ...
  return <div style={{ display: "grid", gap: 16 }}>
    <ContinuityFilterBar query={filterQuery} onQueryChange={setFilterQuery} placeholder="Buscar estrategia o procedimiento…"><select className="nf-app-input" aria-label="Tipo de estrategia" value={filterType} onChange={(e) => setFilterType(e.target.value)}><option value="ALL">Todos los tipos</option>{STRATEGY_TYPE_OPTIONS.map((value) => <option key={value} value={value}>{STRATEGY_TYPE[value]}</option>)}</select><select className="nf-app-input" aria-label="Estado de estrategia" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}><option value="ALL">Todos los estados</option>{Object.entries(STRATEGY_STATUS).map(([value, item]) => <option key={value} value={value}>{item.label}</option>)}</select></ContinuityFilterBar>
    <div>
      <IsoSectionHeader
        icon={ShieldCheck}
      ...
      <div className="nf-data-table-wrap"><table className="nf-data-table" style={{ minWidth: 960 }}>
        ...
        <tbody>{visibleStrategies.map((s) => { const next = STRATEGY_NEXT[s.status]; return <tr key={s.id}>
          ...
          <td>{p.canUpdate && <><button type="button" className="nf-app-btn-ghost" onClick={() => { setEditingStrategy(editingStrategy === s.id ? null : s.id); setStrategyEdit({ title: s.title, activityId: s.activity?.id ?? "", type: s.type, achievesRtoMinutes: s.achievesRtoMinutes != null ? String(s.achievesRtoMinutes) : "", achievesRpoMinutes: s.achievesRpoMinutes != null ? String(s.achievesRpoMinutes) : "", cost: s.cost != null ? String(s.cost) : "", description: s.description ?? "", ownerId: s.owner?.id ?? "", resourcesNeeded: "", notes: "" }); }}>Editar</button> {next && <button type="button" className="nf-app-btn-ghost" disabled={pending} onClick={() => onRun(() => setStrategyStatus({ id: s.id, status: next as never }), { successMessage: "Estado actualizado." })}>{STRATEGY_NEXT_LABEL[next] ?? next}</button>} {s.status === "PROPOSED" && <button type="button" className="nf-app-btn-ghost" disabled={pending} onClick={() => onRun(() => setStrategyStatus({ id: s.id, status: "REJECTED" }), { successMessage: "Estrategia rechazada." })}>Rechazar</button>} {s.status === "IMPLEMENTED" && <button type="button" className="nf-app-btn-ghost" disabled={pending} onClick={() => onRun(() => setStrategyStatus({ id: s.id, status: "RETIRED" }), { successMessage: "Estrategia retirada." })}>Retirar</button>}</>}</td>
        </tr>; })}</tbody></table>
    ...
    <div>
      ...
      <div className="nf-data-table-wrap"><table className="nf-data-table" style={{ minWidth: 860 }}>
        ...
        <tbody>{visibleProcedures.map((r) => <tr key={r.id}>
          ...
          <td>{mins(r.estimatedMinutes)}</td><td>{r.version}</td><td>{p.canUpdate && <button type="button" className="nf-app-btn-ghost" onClick={() => { setEditingProcedure(editingProcedure === r.id ? null : r.id); setProcedureEdit({ title: r.title, activityId: r.activity?.id ?? "", estimatedMinutes: r.estimatedMinutes != null ? String(r.estimatedMinutes) : "", objective: r.objective ?? "", steps: "", prerequisites: "", order: String(r.order), version: r.version }); }}>Editar</button>}</td>
        </tr>)}</tbody></table>
        <Modal open={Boolean(editingProcedureRow)} onClose={() => setEditingProcedure(null)} title={`Editar procedimiento · ${editingProcedureRow?.code ?? ""}`} width={760}>
          <div style={{ display: "grid", gap: 12 }}><label>Título<input className="nf-app-input" maxLength={200} value={procedureEdit.title} onChange={(e) => setProcedureEdit((f) => ({ ...f, title: e.target.value }))} /></label><div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}><label>Actividad<select className="nf-app-input" value={procedureEdit.activityId} onChange={(e) => setProcedureEdit((f) => ({ ...f, activityId: e.target.value }))}><option value="">Sin actividad</option>{p.activities.map((a) => <option key={a.id} value={a.id}>{a.code} · {a.name}</option>)}</select></label><label>Duración<input className="nf-app-input" type="number" min={0} max={MAX_MINUTES} value={procedureEdit.estimatedMinutes} onChange={(e) => setProcedureEdit((f) => ({ ...f, estimatedMinutes: e.target.value }))} /></label><label>Versión<input className="nf-app-input" maxLength={20} value={procedureEdit.version} onChange={(e) => setProcedureEdit((f) => ({ ...f, version: e.target.value }))} /></label></div><label>Objetivo<textarea className="nf-app-input" maxLength={4000} rows={2} value={procedureEdit.objective} onChange={(e) => setProcedureEdit((f) => ({ ...f, objective: e.target.value }))} /></label><label>Pasos<textarea className="nf-app-input" maxLength={20000} rows={3} value={procedureEdit.steps} onChange={(e) => setProcedureEdit((f) => ({ ...f, steps: e.target.value }))} /></label><label>Prerrequisitos<textarea className="nf-app-input" maxLength={4000} rows={2} value={procedureEdit.prerequisites} onChange={(e) => setProcedureEdit((f) => ({ ...f, prerequisites: e.target.value }))} /></label><div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}><button type="button" className="nf-app-btn-ghost" onClick={() => setEditingProcedure(null)}>Cancelar</button><button type="button" className="nf-app-btn-primary" disabled={pending || !editingProcedureRow || !procedureEdit.title.trim()} onClick={() => editingProcedureRow && onRun(() => updateRecoveryProcedure({ id: editingProcedureRow.id, title: requiredText(procedureEdit.title, "El título", 200), activityId: assertOptions(procedureEdit.activityId, p.activities.map((a) => a.id), "La actividad") || null, estimatedMinutes: optionalNumber(procedureEdit.estimatedMinutes, "La duración"), objective: optionalText(procedureEdit.objective, "El objetivo", 4000) ?? null, steps: optionalText(procedureEdit.steps, "Los pasos", 20000) ?? null, prerequisites: optionalText(procedureEdit.prerequisites, "Los prerrequisitos", 4000) ?? null, order: optionalNumber(procedureEdit.order, "El orden") ?? 0, version: requiredText(procedureEdit.version, "La versión", 20) }), { onSuccess: () => setEditingProcedure(null), successMessage: "Procedimiento actualizado." })}>Guardar cambios</button></div></div>
        </Modal>
  ...
  return <div style={{ border: "1px solid var(--nf-line)", borderRadius: 10, padding: 14 }}>
    ...
    <div className="nf-data-table-wrap" style={{ marginTop: 10 }}><table className="nf-data-table" style={{ minWidth: 780 }}>
      ...
      <tbody>{t.contacts.map((c) => <tr key={c.id}>
        ...
        <td>{c.primaryPhone ?? "—"}</td><td>{c.altPhone ?? "—"}</td><td>{c.email ?? "—"}</td><td>{p.canUpdate && <button type="button" className="nf-app-btn-ghost" onClick={() => { setEditingContact(editingContact === c.id ? null : c.id); setContactEdit({ name: c.name, role: c.role ?? "", type: c.type, primaryPhone: c.primaryPhone ?? "", altPhone: c.altPhone ?? "", email: c.email ?? "", escalationOrder: String(c.escalationOrder), isDeputy: c.isDeputy, availability: "", notes: "" }); }}>Editar</button>}</td>
      </tr>)}</tbody></table>
...
function GapsTab({ p }: { p: ContinuityPayload }) {
  ...
  return <div>
    <ContinuityFilterBar query={filterQuery} onQueryChange={setFilterQuery} placeholder="Buscar actividad o detalle…"><select className="nf-app-input" aria-label="Tipo de brecha" value={filterKind} onChange={(e) => setFilterKind(e.target.value)}><option value="ALL">Todas las brechas</option>{Object.entries(GAP_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></ContinuityFilterBar>
