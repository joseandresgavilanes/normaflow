...
export default function SecurityControlsLiveClient({ initial }: { initial: SecurityControlsPayload }) {
  ...
  const columns = useMemo<DataTableColumn<Row>[]>(() => [
    ...
    { id: "implementation", header: "Implementación", minWidth: 130, numeric: true, sortValue: (r) => r.implementationLevel,
      cell: (r) => <div style={{ minWidth: 110 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span>{r.implementationLevel}%</span><span>{r.status === "EFFECTIVE" ? "✓" : ""}</span></div><div style={{ height: 6, background: "#edf0f5", borderRadius: 9, marginTop: 4 }}><div style={{ width: `${r.implementationLevel}%`, height: "100%", background: r.status === "EFFECTIVE" ? "#15803D" : "#5266F6", borderRadius: 9 }} /></div></div> },
    ...
    <div className="nf-modal-backdrop" role="dialog" aria-modal="true">
      <div className="nf-modal" style={{ maxWidth: 760, width: "calc(100% - 32px)", maxHeight: "90vh", overflow: "auto" }}>
        ...
        <div style={{ display: "grid", gap: 16, padding: 20 }}>
          ...
          {canUpdate && <div style={{ borderTop: "1px solid var(--nf-line)", paddingTop: 14 }}><h4>Vincular trazabilidad</h4><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><select aria-label="Evidencia" className="nf-app-input" value={evidenceId} onChange={(event) => setEvidenceId(event.target.value)}><option value="">Evidencia…</option>{initial.evidenceOptions.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><button type="button" className="nf-app-btn-ghost" disabled={pending || !evidenceId} onClick={() => onRun(() => linkControlEvidence({ organizationControlId: row.id, evidenceId, period: new Date().toISOString().slice(0, 7) }))}><Link2 size={14} /> Vincular evidencia</button><select aria-label="Riesgo" className="nf-app-input" value={riskId} onChange={(event) => setRiskId(event.target.value)}><option value="">Riesgo…</option>{initial.riskOptions.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><button type="button" className="nf-app-btn-ghost" disabled={pending || !riskId} onClick={() => onRun(() => linkRiskToSecurityControl({ organizationControlId: row.id, riskId, purpose: "Tratamiento del riesgo relacionado" }))}><Link2 size={14} /> Vincular riesgo</button></div></div>}
          {canApprove && <div style={{ borderTop: "1px solid var(--nf-line)", paddingTop: 14 }}><h4>Revisión de eficacia</h4><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><select aria-label="Revisión" className="nf-app-input" value={review} onChange={(event) => setReview(event.target.value)}><option value="CONFORMING">Conforme</option><option value="PARTIALLY_CONFORMING">Parcial</option><option value="NONCONFORMING">No conforme</option></select><select aria-label="Eficacia" className="nf-app-input" value={effectiveness} onChange={(event) => setEffectiveness(event.target.value)}><option value="EFFECTIVE">Efectivo</option><option value="INEFFECTIVE">No efectivo</option><option value="NOT_TESTED">No probado</option></select><button type="button" className="nf-app-btn-primary" disabled={pending} onClick={() => onRun(() => reviewSecurityControl({ organizationControlId: row.id, result: review, effectiveness, comments: "Revisión registrada desde el catálogo operativo." }))}><ShieldCheck size={14} /> Registrar revisión</button></div></div>}
