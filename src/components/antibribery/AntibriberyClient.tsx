  ...
  return (
    <div className="nf-iso-module" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      {tab === "panel" ? <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 12 }}>
        ...
        <Stat label="DD abiertas" value={s.dueDiligenceOpen} accent={s.dueDiligenceOpen ? "var(--nf-warning-text)" : undefined} />
        <Stat label="Regalos pendientes" value={s.giftsPending} accent={s.giftsPending ? "var(--nf-warning-text)" : undefined} />
        <Stat label="Investigaciones abiertas" value={s.investigationsOpen} accent={s.investigationsOpen ? "var(--nf-warning-text)" : undefined} />
      </div> : <IsoSectionMetrics items={tab === "risks" ? [{ label: "Evaluaciones", value: s.assessments }, { label: "Riesgo residual alto", value: s.highResidual, accent: s.highResidual ? "var(--nf-danger-text)" : undefined }, { label: "Aprobadas", value: s.assessmentsApproved }] : tab === "associates" ? [{ label: "Socios de negocio", value: s.associates }, { label: "Riesgo alto/crítico", value: s.highRiskAssociates, accent: s.highRiskAssociates ? "var(--nf-warning-text)" : undefined }, { label: "Debida diligencia abierta", value: s.dueDiligenceOpen, accent: s.dueDiligenceOpen ? "var(--nf-warning-text)" : undefined }] : tab === "due-diligence" ? [{ label: "Casos abiertos", value: s.dueDiligenceOpen, accent: s.dueDiligenceOpen ? "var(--nf-warning-text)" : undefined }, { label: "Vencidos", value: s.dueDiligenceOverdue, accent: s.dueDiligenceOverdue ? "var(--nf-danger-text)" : undefined }, { label: "Socios evaluados", value: s.associates }] : tab === "owners" ? [{ label: "Beneficiarios finales", value: initial.owners.length }, { label: "PEP", value: s.pepOwners, accent: s.pepOwners ? "var(--nf-warning-text)" : undefined }, { label: "Socios de negocio", value: s.associates }] : tab === "gifts" ? [{ label: "Regalos pendientes", value: s.giftsPending, accent: s.giftsPending ? "var(--nf-warning-text)" : undefined }, { label: "Regalos registrados", value: initial.gifts.length }, { label: "Socios relacionados", value: s.associates }] : tab === "donations" ? [{ label: "Donaciones", value: initial.donations.length }, { label: "Políticas", value: s.donationsPolitical, accent: s.donationsPolitical ? "var(--nf-warning-text)" : undefined }, { label: "Pendientes", value: initial.donations.filter((row) => !["APPROVED", "REJECTED"].includes(row.status)).length, accent: "var(--nf-warning)" }] : tab === "conflicts" ? [{ label: "Declaraciones", value: initial.conflicts.length }, { label: "Pendientes", value: initial.conflicts.filter((row) => row.reviewStatus === "PENDING").length, accent: "var(--nf-warning)" }, { label: "Socios relacionados", value: s.associates }] : tab === "facilitation" ? [{ label: "Pagos abiertos", value: s.facilitationOpen, accent: s.facilitationOpen ? "var(--nf-danger-text)" : undefined }, { label: "Reportes", value: initial.facilitation.length }, { label: "Investigaciones", value: s.investigationsOpen, accent: s.investigationsOpen ? "var(--nf-warning-text)" : undefined }] : tab === "controls" ? [{ label: "Pruebas financieras", value: initial.financialTests.length }, { label: "Pruebas no financieras", value: initial.nonFinancialTests.length }, { label: "Fallos de control", value: s.controlFailures, accent: s.controlFailures ? "var(--nf-danger-text)" : undefined }] : tab === "approvals" ? [{ label: "Aprobaciones pendientes", value: s.highRiskPending, accent: s.highRiskPending ? "var(--nf-warning-text)" : undefined }, { label: "Operaciones alto riesgo", value: initial.highRisk.length }, { label: "Socios", value: s.associates }] : tab === "commitments" ? [{ label: "Compromisos", value: s.commitments }, { label: "Socios relacionados", value: s.associates }, { label: "Investigaciones", value: s.investigationsOpen, accent: s.investigationsOpen ? "var(--nf-warning-text)" : undefined }] : [{ label: "Investigaciones", value: initial.investigations.length }, { label: "Abiertas", value: s.investigationsOpen, accent: s.investigationsOpen ? "var(--nf-warning-text)" : undefined }, { label: "Socios relacionados", value: s.associates }]} />}
