...
export default function SoALiveClient({ initial }: { initial: SoAPayload }) {
  ...
  const columns = useMemo<DataTableColumn<Entry>[]>(() => [
    ...
    { id: "justification", header: "Justificación", minWidth: 220, sortValue: (r) => r.justification ?? "",
      cell: (r) => <span style={{ fontSize: 12, color: "var(--nf-ink-3)" }}>{r.applicability === "EXCLUDED" ? (r.justification || <span style={{ color: "var(--nf-danger-text)" }}>Falta justificación</span>) : "—"}</span> },
      ...
      <div className="nf-modal" style={{ maxWidth: 680, width: "calc(100% - 32px)", maxHeight: "90vh", overflow: "auto" }}>
        ...
        <div style={{ display: "grid", gap: 14, padding: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <label>Aplicabilidad<Picker aria-label="Aplicabilidad" className="nf-app-input" value={app} onChange={(e) => setApp(e.target.value as typeof app)}>{Object.entries(APPLICABILITY_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Picker></label>
            <label>Estado de implementación<Picker aria-label="Estado de implementación" className="nf-app-input" value={status} onChange={(e) => setStatus(e.target.value as typeof status)}>{Object.entries(STATUS_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Picker></label>
          </div>
          <label>Justificación {app === "EXCLUDED" && <span style={{ color: "var(--nf-danger-text)" }}>· obligatoria para exclusiones</span>}<textarea className="nf-app-input" rows={3} value={justification} onChange={(e) => setJustification(e.target.value)} placeholder={app === "EXCLUDED" ? "Motivo de la exclusión del control…" : "Justificación de la decisión de aplicabilidad…"} /></label>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <label>Riesgo relacionado<Picker aria-label="Riesgo relacionado" className="nf-app-input" value={riskItemId} onChange={(e) => setRiskItemId(e.target.value)}><option value="">Ninguno</option>{initial.riskItemOptions.map((r) => <option key={r.id} value={r.id}>{r.reference} · {r.title}</option>)}</Picker></label>
          </div>
          <label>Evidencia<Picker aria-label="Evidencia" className="nf-app-input" value={evidenceId} onChange={(e) => setEvidenceId(e.target.value)}><option value="">Sin evidencia</option>{initial.evidenceOptions.map((ev) => <option key={ev.id} value={ev.id}>{ev.title}</option>)}</Picker></label>
