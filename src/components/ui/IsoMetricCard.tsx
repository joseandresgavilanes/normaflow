...
const METRIC_STYLES = [
  { match: /riesgo|peligro|incidente|vencid|abiert|no conform|fallo|rechaz/i, Icon: AlertTriangle, color: "#B91C1C", background: "#fff0f0" },
  { match: /cumpl|control|cobertura|aprob|eficaz|activo|evaluad/i, Icon: ShieldCheck, color: "#15803D", background: "#eaf7ef" },
  { match: /objetiv|accion|mejora|ahorro|oportun/i, Icon: Target, color: "#B45309", background: "#fff8e6" },
  { match: /proveedor|socio|tercero|personal|usuario/i, Icon: Handshake, color: "#6B3FB5", background: "#f5efff" },
  { match: /producto|dispositivo|sistema|servicio|lote|dataset|modelo/i, Icon: Boxes, color: "#0E7490", background: "#ecfeff" },
  { match: /evidencia|document|registro|obligacion|requisito/i, Icon: FileCheck2, color: "#5266F6", background: "#eef0ff" },
...
export default function IsoMetricCard({ label, value, suffix, accent }: { label: string; value: string | number; suffix?: string; accent?: string }) {
  const style = METRIC_STYLES.find((item) => item.match.test(label)) ?? { Icon: Activity, color: "#5266F6", background: "#eef0ff" };
