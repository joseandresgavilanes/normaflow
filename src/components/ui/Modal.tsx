...
export default function Modal({ open, onClose, title, children, width = 560 }: ModalProps) {
  ...
  if (!open) return null;
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(14,28,50,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: width, maxHeight: "88vh", overflow: "auto", boxShadow: "0 24px 80px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
