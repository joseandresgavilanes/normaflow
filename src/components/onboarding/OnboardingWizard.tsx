...
function Choice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} style={{ textAlign: "left", border: `1px solid ${selected ? "var(--nf-primary)" : "var(--nf-border)"}`, background: selected ? "var(--nf-primary-subtle)" : "#fff", borderRadius: 14, padding: 16, cursor: "pointer", color: "var(--nf-text-primary)", boxShadow: selected ? "0 0 0 3px rgba(82,102,246,.10)" : "none" }}>{children}</button>;
}
...
export default function OnboardingWizard({ initial, userName, needsOrganization }: Props) {
  ...
  return <main style={{ minHeight: "100vh", background: "linear-gradient(135deg,#f7f9fc 0%,#eef2ff 100%)", padding: "32px 20px 56px", color: "var(--nf-text-primary)" }}>
    <div style={{ maxWidth: 1040, margin: "0 auto" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, marginBottom: 28 }}>
        <Link href="/home" style={{ display: "inline-flex", alignItems: "center", gap: 9, textDecoration: "none", color: "var(--nf-text-primary)", fontSize: 20, fontWeight: 750 }}><span style={{ width: 34, height: 34, display: "grid", placeItems: "center", borderRadius: 10, background: "var(--nf-primary)", color: "#fff", fontWeight: 900 }}>N</span> NormaFlow</Link>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 280px", gap: 22, alignItems: "start" }}>
        <Card style={{ padding: "30px clamp(20px,5vw,52px) 34px" }}>
          ...
          {step === 4 && <section><div style={{ display: "flex", gap: 13, alignItems: "flex-start", padding: 16, background: "#f0fbf4", border: "1px solid #b8e4c4", borderRadius: 14, marginBottom: 22 }}><span style={{ width: 34, height: 34, background: "var(--nf-success)", color: "#fff", borderRadius: 10, display: "grid", placeItems: "center" }}><Rocket size={18} /></span><span><strong style={{ display: "block", color: "var(--nf-success-text)" }}>Tu workspace está listo</strong><span style={{ color: "var(--nf-success-text)", fontSize: 13 }}>Normas, cláusulas GAP, procesos base y plantillas iniciales preparados.</span></span></div><h2 style={{ fontSize: 22, margin: "0 0 6px" }}>Tu checklist de activación</h2><p style={{ color: "var(--nf-text-secondary)", fontSize: 14, margin: "0 0 16px" }}>Completa estos pasos para obtener valor real del trial. El progreso se actualiza solo al trabajar en cada módulo.</p>{initial ? <><div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 15 }}><div style={{ flex: 1, height: 8, borderRadius: 8, background: "#e8edf5", overflow: "hidden" }}><div style={{ height: "100%", width: `${progress}%`, background: progress >= 70 ? "var(--nf-success)" : "var(--nf-primary)", borderRadius: 8 }} /></div><strong style={{ fontSize: 13, color: "var(--nf-primary-active)" }}>{progress}%</strong></div><div style={{ display: "grid", gap: 8 }}>{checklist.map((item) => <Link key={item.id} href={item.href} style={{ textDecoration: "none", color: "inherit", display: "flex", alignItems: "center", gap: 11, padding: "12px 13px", border: "1px solid var(--nf-border)", borderRadius: 12, background: item.done ? "#f7fcf8" : "#fff" }}><span style={{ width: 22, height: 22, borderRadius: 7, display: "grid", placeItems: "center", background: item.done ? "var(--nf-success)" : "var(--nf-surface-muted)", color: item.done ? "#fff" : "#8794a5" }}>{item.done ? <Check size={14} /> : <ArrowRight size={14} />}</span><span style={{ flex: 1 }}><strong style={{ display: "block", fontSize: 13 }}>{item.title}</strong><span style={{ color: "var(--nf-text-secondary)", fontSize: 12 }}>{item.description}</span></span><ArrowRight size={15} color="#9aa6b5" /></Link>)}</div></> : <div style={{ padding: 16, background: "var(--nf-surface-muted)", borderRadius: 12, color: "var(--nf-text-secondary)", fontSize: 13 }}>Actualiza la pantalla para cargar tu checklist.</div>}<div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 24 }}><button type="button" onClick={enterWorkspace} disabled={pending} style={primaryButton}>{pending ? "Guardando…" : "Entrar al workspace"}<ArrowRight size={16} /></button><button type="button" onClick={skip} disabled={pending} style={secondaryButton}>Continuar después</button></div></section>}
        ...
        <aside style={{ display: "grid", gap: 14 }}><Card style={{ padding: 18, background: "#142033", color: "#fff", border: "none" }}><div style={{ display: "flex", gap: 9, alignItems: "center", color: "#aebaff", fontSize: 12, fontWeight: 800, textTransform: "uppercase" }}><Gauge size={15} /> Tu trial</div><div style={{ fontSize: 34, fontWeight: 850, margin: "12px 0 2px" }}>{days} <span style={{ fontSize: 14, fontWeight: 600 }}>días</span></div><p style={{ fontSize: 12, lineHeight: 1.5, color: "#c5cfdf", margin: 0 }}>Explora el sistema completo durante 14 días. No te pedimos tarjeta para empezar.</p></Card><Card style={{ padding: 18 }}><strong style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 14 }}><Users size={16} color="#5266F6" /> Diseñado para tu equipo</strong><p style={{ color: "var(--nf-text-secondary)", fontSize: 12, lineHeight: 1.5, margin: "9px 0 0" }}>Invita responsables, auditores y revisores cuando tu base esté lista.</p></Card><Card style={{ padding: 18 }}><strong style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 14 }}><CircleHelp size={16} color="#5266F6" /> ¿Necesitas ayuda?</strong><p style={{ color: "var(--nf-text-secondary)", fontSize: 12, lineHeight: 1.5, margin: "9px 0 0" }}>Empieza por el GAP y luego trabaja la evidencia de las brechas prioritarias.</p></Card></aside>
      </div>
...
function UpgradePanel({ initial, pending }: { initial: OnboardingPayload | null; pending: boolean }) {
  ...
  return <Card style={{ marginTop: 22, padding: 22 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}><div><div style={{ color: "var(--nf-primary-active)", fontSize: 12, fontWeight: 800, textTransform: "uppercase" }}>Cuando estés listo</div><h2 style={{ fontSize: 21, margin: "7px 0 5px" }}>Conserva tu progreso después del trial</h2><p style={{ color: "var(--nf-text-secondary)", fontSize: 13, margin: 0 }}>Elige el plan que encaje con tu equipo. Puedes ampliar después.</p></div><Link href="/app/billing" style={{ color: "var(--nf-primary-active)", fontWeight: 700, fontSize: 13 }}>Ver facturación →</Link></div><div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 10, marginTop: 16 }}>{initial.billing.plans.map((plan) => <div key={plan.key} style={{ border: "1px solid var(--nf-border)", borderRadius: 12, padding: 13 }}><strong style={{ fontSize: 14 }}>{plan.name}</strong><div style={{ color: "var(--nf-primary-active)", fontSize: 20, fontWeight: 850, margin: "7px 0" }}>{plan.price == null ? "A medida" : `$${plan.price}`}<small style={{ color: "var(--nf-text-secondary)", fontSize: 11 }}>{plan.price == null ? "" : " USD/mes"}</small></div><button type="button" disabled={busy || pending || !initial.billing.canManage || !initial.billing.stripeConfigured || (plan.key !== "ENTERPRISE" && !initial.billing.checkoutConfigured[plan.key as "STARTER" | "GROWTH"])} onClick={() => plan.key !== "ENTERPRISE" && upgrade(plan.key as "STARTER" | "GROWTH")} style={plan.key === "GROWTH" ? primaryButton : secondaryButton}>{plan.key === "ENTERPRISE" ? "Hablar con ventas" : initial.billing.stripeConfigured ? "Elegir plan" : "Configura Stripe"}</button></div>)}</div>{error && <p role="alert" style={{ color: "#a52e2e", fontSize: 12, marginBottom: 0 }}>{error}</p>}</Card>;
}
