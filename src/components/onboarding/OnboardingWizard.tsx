...
function Choice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} style={{ textAlign: "left", border: `1px solid ${selected ? "var(--nf-primary)" : "#e5eaf2"}`, background: selected ? "#f3f5ff" : "#fff", borderRadius: 14, padding: 16, cursor: "pointer", color: "#142033", boxShadow: selected ? "0 0 0 3px rgba(82,102,246,.10)" : "none" }}>{children}</button>;
}
  ...
  return <main style={{ minHeight: "100vh", background: "linear-gradient(135deg,#f7f9fc 0%,#eef2ff 100%)", padding: "32px 20px 56px", color: "#142033" }}>
    <div style={{ maxWidth: 1040, margin: "0 auto" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, marginBottom: 28 }}>
        <Link href="/home" style={{ display: "inline-flex", alignItems: "center", gap: 9, textDecoration: "none", color: "#142033", fontSize: 20, fontWeight: 750 }}><span style={{ width: 34, height: 34, display: "grid", placeItems: "center", borderRadius: 10, background: "var(--nf-primary)", color: "#fff", fontWeight: 900 }}>N</span> NormaFlow</Link>
...
function UpgradePanel({ initial, pending }: { initial: OnboardingPayload | null; pending: boolean }) {
  ...
  return <Card style={{ marginTop: 22, padding: 22 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}><div><div style={{ color: "var(--nf-primary)", fontSize: 12, fontWeight: 800, textTransform: "uppercase" }}>Cuando estés listo</div><h2 style={{ fontSize: 21, margin: "7px 0 5px" }}>Conserva tu progreso después del trial</h2><p style={{ color: "#5e6b7a", fontSize: 13, margin: 0 }}>Elige el plan que encaje con tu equipo. Puedes ampliar después.</p></div><Link href="/app/billing" style={{ color: "var(--nf-primary)", fontWeight: 700, fontSize: 13 }}>Ver facturación →</Link></div><div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 10, marginTop: 16 }}>{initial.billing.plans.map((plan) => <div key={plan.key} style={{ border: "1px solid #e5eaf2", borderRadius: 12, padding: 13 }}><strong style={{ fontSize: 14 }}>{plan.name}</strong><div style={{ color: "var(--nf-primary)", fontSize: 20, fontWeight: 850, margin: "7px 0" }}>{plan.price == null ? "A medida" : `$${plan.price}`}<small style={{ color: "#5e6b7a", fontSize: 11 }}>{plan.price == null ? "" : " USD/mes"}</small></div><button type="button" disabled={busy || pending || !initial.billing.canManage || !initial.billing.stripeConfigured || (plan.key !== "ENTERPRISE" && !initial.billing.checkoutConfigured[plan.key as "STARTER" | "GROWTH"])} onClick={() => plan.key !== "ENTERPRISE" && upgrade(plan.key as "STARTER" | "GROWTH")} style={plan.key === "GROWTH" ? primaryButton : secondaryButton}>{plan.key === "ENTERPRISE" ? "Hablar con ventas" : initial.billing.stripeConfigured ? "Elegir plan" : "Configura Stripe"}</button></div>)}</div>{error && <p role="alert" style={{ color: "#a52e2e", fontSize: 12, marginBottom: 0 }}>{error}</p>}</Card>;
}
...
const primaryButton: React.CSSProperties = { border: "none", borderRadius: 10, background: "var(--nf-primary)", color: "#fff", padding: "11px 16px", fontSize: 13, fontWeight: 750, display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer" };
const secondaryButton: React.CSSProperties = { border: "1px solid #dbe2ec", borderRadius: 10, background: "#fff", color: "var(--nf-primary)", padding: "10px 14px", fontSize: 13, fontWeight: 750, display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer" };
const backStyle: React.CSSProperties = { border: "none", background: "transparent", color: "var(--nf-primary)", padding: 0, fontSize: 13, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 3, cursor: "pointer" };
