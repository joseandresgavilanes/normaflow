...
export function EvidenceRepositoryLiveClient({ initial }: { initial: EvidencePayload }) {
  ...
  const filtered = useMemo(() => {
    ...
    return initial.evidence.filter((row) => {
      ...
      return [row.title, row.description, row.processName, row.standardCode, row.clauseName, row.responsibleName, ...row.documentLabels, ...row.riskLabels, ...row.auditLabels, ...row.auditProgramLabels, ...row.nonconformityLabels]
  ...
  const columns = useMemo<DataTableColumn<EvidenceRow>[]>(() => [
    ...
    { id: "actions", header: "Acciones", minWidth: 150, hideable: false,
      cell: (r) => <div className="nf-row-actions">
        <RowAction icon={previewBusy === r.id ? Loader2 : Eye} label="Previsualizar" tone="primary" disabled={previewBusy === r.id} onClick={() => { void openPreview(r); }} />
      </div> },
...
function EvidenceCreateModal({ initial, open, isPending, onClose, onSubmit }: { initial: EvidencePayload; open: boolean; isPending: boolean; onClose: () => void; onSubmit: (input: CreateEvidenceInput) => void }) {
  return <Modal open={open} onClose={onClose} title="Subir evidencia" width={760}><form className="nf-modal-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const file = form.get("file"); if (!(file instanceof File) || !file.size) return; const input: CreateEvidenceInput = { title: String(form.get("title") ?? ""), description: String(form.get("description") ?? ""), evidenceType: String(form.get("evidenceType") ?? "OTHER") as EvidenceType, processId: String(form.get("processId") ?? "") || undefined, standardCode: String(form.get("standardCode") ?? "") || undefined, clauseId: String(form.get("clauseId") ?? "") || undefined, responsibleId: String(form.get("responsibleId") ?? "") || undefined, issuedAt: String(form.get("issuedAt") ?? "") || undefined, expiresAt: String(form.get("expiresAt") ?? "") || undefined, file, links: { documentIds: form.getAll("documentIds").map(String), riskIds: form.getAll("riskIds").map(String), auditIds: form.getAll("auditIds").map(String), auditProgramIds: form.getAll("auditProgramIds").map(String), findingIds: form.getAll("findingIds").map(String), nonconformityIds: form.getAll("nonconformityIds").map(String), indicatorIds: form.getAll("indicatorIds").map(String), managementReviewIds: form.getAll("managementReviewIds").map(String) } }; onSubmit(input); }} style={{ display: "grid", gap: 13 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}><ClausePicker clauses={initial.clauses} standardName="standardCode" labelClause="Cláusula" inputClassName={NF_INPUT_CLASS} inputStyle={modalInputStyle} /><Field label="Proceso"><Picker aria-label="Proceso" name="processId" className={NF_INPUT_CLASS} style={modalInputStyle}><option value="">—</option>{initial.targets.process.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Picker></Field></div>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}><Field label="Responsable"><PersonPicker name="responsibleId" people={initial.members} placeholder="Sin asignar" ariaLabel="Responsable" style={modalInputStyle} /></Field><Field label="Fecha de emisión"><DateField aria-label="Fecha de emisión" name="issuedAt" className={NF_INPUT_CLASS} style={modalInputStyle} /></Field><Field label="Fecha de vencimiento"><DateField aria-label="Fecha de caducidad" name="expiresAt" className={NF_INPUT_CLASS} style={modalInputStyle} /></Field></div>
    <FileImportArea name="file" required accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.csv,.txt,.md,image/*" maxSizeMB={50} label="Archivo *" hint="Se guarda en el repositorio privado de la organización." compact />
    <div><div style={{ fontSize: 12, fontWeight: 700, color: "var(--nf-ink-2)", marginBottom: 7 }}>Vincular con registros</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>{(["documentIds", "riskIds", "auditIds", "auditProgramIds", "findingIds", "nonconformityIds", "indicatorIds", "managementReviewIds"] as const).map((name) => <MultiSelect key={name} name={name} label={linkLabel(name)} options={initial.targets[targetKey(name)]} />)}</div></div>
...
function EvidenceDetailModal({ row, canReview, canDelete, isPending, onClose, onPreview, onReview, onArchive }: { row: EvidenceRow | null; canReview: boolean; canDelete: boolean; isPending: boolean; onClose: () => void; onPreview: () => void; onReview: (decision: "APPROVE" | "REJECT") => void; onArchive: () => void }) {
  if (!row) return null;
  const links = [...row.documentLabels, ...row.riskLabels, ...row.auditLabels, ...row.auditProgramLabels, ...row.findingLabels, ...row.nonconformityLabels, ...row.indicatorLabels, ...row.managementReviewLabels];
...
function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: { value: string; label: string }[] }) { return <Picker aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className={NF_INPUT_CLASS} style={{ width: "auto", minWidth: 140 }}>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Picker>; }
function MultiSelect({ name, label, options }: { name: string; label: string; options: { id: string; label: string }[] }) { return <label style={{ display: "grid", gap: 5, fontSize: 11, color: "var(--nf-ink-3)" }}>{label}<Picker aria-label={label} name={name} multiple className={NF_INPUT_CLASS} style={modalInputStyle}>{options.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</Picker></label>; }
function linkLabel(name: string) { return ({ documentIds: "Documentos", riskIds: "Riesgos", auditIds: "Auditorías", auditProgramIds: "Programa de auditorías", findingIds: "Hallazgos", nonconformityIds: "CAPA / NC", indicatorIds: "Indicadores", managementReviewIds: "Revisión por dirección" } as Record<string, string>)[name] ?? name; }
function targetKey(name: string): keyof EvidencePayload["targets"] { return ({ documentIds: "document", riskIds: "risk", auditIds: "audit", auditProgramIds: "auditProgram", findingIds: "finding", nonconformityIds: "nc", indicatorIds: "indicator", managementReviewIds: "managementReview" } as Record<string, keyof EvidencePayload["targets"]>)[name]; }
