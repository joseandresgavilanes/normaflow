import { z } from "zod";
import { ACPMStage, ActionType, AuditStatus, AuditType, DocumentStatus, DocumentType, IndicatorStatus, ManagementReviewStatus, ManagementReviewTopic, NCSeverity, NCSource, NCStatus, Priority, RiskStatus, RiskTreatment } from "@prisma/client";
import { boundedArray, dateInputSchema, finiteNumber, idSchema, optionalDateInputSchema, optionalText, shortText } from "./common";

const optionalId = idSchema.optional().nullable();
...
export const documentReviewSchema = z.object({ approverIds: boundedArray(idSchema, 50).min(1) });
export const documentContentSchema = z.object({ content: shortText(500_000), changeDescription: shortText(1000), bump: z.enum(["minor", "major"]).optional() });

export const acpmCreateSchema = z.object({ title: shortText(240), description: optionalText(8000), type: z.nativeEnum(ActionType), priority: z.nativeEnum(Priority), source: optionalText(160), dueDate: optionalDateInputSchema, ownerId: optionalId });
...
export const managementReviewSchema = z.object({ title: shortText(240), scheduledDate: dateInputSchema, chairId: idSchema, attendees: boundedArray(shortText(160), 100).optional(), participantIds: boundedArray(idSchema, 100).optional(), standards: boundedArray(standardCode, 10).min(1) });
export const managementReviewUpdateSchema = managementReviewSchema.partial().extend({ heldAt: optionalDateInputSchema, status: z.nativeEnum(ManagementReviewStatus).optional(), summary: optionalText(8000) });
export const reviewInputSchema = z.object({ topic: z.nativeEnum(ManagementReviewTopic), content: shortText(8000), auditId: optionalId, indicatorId: optionalId, riskId: optionalId, nonconformityId: optionalId, actionId: optionalId, capaId: optionalId });
export const reviewDecisionSchema = z.object({ topic: shortText(160), decision: shortText(8000), ownerId: optionalId, dueDate: optionalDateInputSchema });
export const reviewStatusSchema = z.nativeEnum(ManagementReviewStatus);
export const nonconformityInputSchema = z.object({ title: shortText(240), description: optionalText(8000), source: z.nativeEnum(NCSource), severity: z.nativeEnum(NCSeverity), status: z.nativeEnum(NCStatus), ownerId: optionalId, rootCause: optionalText(8000), dueDate: optionalDateInputSchema, auditId: optionalId, findingId: optionalId, effectivenessValidated: z.boolean().optional() });
