    ...
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 560px; margin: 0 auto;">
        ...
        <div style="background: #fff; padding: 32px; border: 1px solid #E5EAF2; border-radius: 0 0 12px 12px;">
          <h2 style="color: #142033;">Bienvenido, ${escapeHtml(name)}</h2>
  ...
  return resend.emails.send({
    ...
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px;">
        <p style="color: #142033; font-weight: 600;">${escapeHtml(title)}</p>
        <p style="color: #5E6B7A; line-height: 1.6;">${escapeHtml(body)}</p>
        ${link ? `<a href="${escapeHtml(safeAppUrl(link))}" style="color: #123C66; font-weight: 600;">Ver detalles →</a>` : ""}
      </div>
    `,
  });
}
