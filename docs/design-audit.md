...
| `AppSidebar` | Las ~143 secciones de norma conviven en la columna global cuando hay varias normas desplegadas | Sub-navegación de módulo mezclada con navegación de producto | **P1** | Anidarlas bajo la norma **activa** (máx. 15 visibles) |
| Módulos de norma | — | Repetir las secciones como pestañas de página deja dos jerarquías para la misma decisión, con riesgo de desincronizarse | — | **Decidido: la navegación entre secciones es solo del sidebar** (`?section=` leído con `useModuleSection`). Las páginas no pintan barra de pestañas |
