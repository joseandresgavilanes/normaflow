...
**S5 — La navegación no escala al catálogo de producto, y la mitad no navega.**
El sidebar muestra **35 enlaces planos sin agrupar**, seguidos de 11 grupos colapsables de norma (con hasta 15 subitems cada uno) y 3 grupos de administración. Son ~180 destinos en una sola columna con scroll. No hay agrupación semántica, ni favoritos, ni búsqueda de navegación, ni breadcrumbs (0 en toda la app), ni paleta de comandos. El estado de los grupos no persiste entre navegaciones.

Y hay algo peor que la densidad: **~143 de esos enlaces no llevan a ninguna parte.** Los subitems de norma apuntan a `/app/{norma}?section=X`, pero **ninguno de los 11 clientes de norma lee el parámetro** (`useSearchParams` = 0 en `EnergyClient`, `ComplianceClient`, `SafetyClient`, `AimsClient`, `ContinuityLiveClient`, `FoodSafetyClient`, `ItsmClient`, `MedicalDevicesClient`, `EnvironmentClient`, `AntibriberyClient`, `IntegratedClient`). Cada uno mantiene su pestaña en estado interno inicializado a `panel`. Es decir: pulsar “ISO 22301 → Estrategias” cambia la URL y vuelve a renderizar el Panel. El sidebar además calcula `isItemActive` con ese parámetro, así que **resalta como activa una sección que nunca se abrió**.
