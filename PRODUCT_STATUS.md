...
> **Standard Pack Engine (2026-07-24):** las normas ahora se añaden como **datos** (paquetes versionados) sin reescribir la app. `Standard→StandardEdition`, `Clause→StandardRequirement`, más `StandardFamily/StandardPack/RequirementMapping/RequirementEvidenceRule/GapQuestionTemplate/AuditChecklistTemplate/StandardTemplate` y la tela de satisfacción compartida `RequirementCoverage` (un documento/riesgo/evidencia satisface requisitos de varias normas). Migración `20260724120000_standard_pack_engine` (rename data-preserving expand→backfill→contract, verificada sobre Postgres desechable). Motor: `src/lib/standard-packs/*`, acciones `src/lib/actions/{standards,standard-packs,requirement-coverage}.ts`, UI `/app/standards`. Formato de paquete: [`docs/standard-packs.md`](docs/standard-packs.md). Tests: `npm run test:packs` (integración) + `tests-live/standards-engine-tenant.spec.ts` (RLS cross-tenant).
